...
import "./styles.css"
import { useTheme } from "@navi-ai/ui/theme"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.NAVI_CHANNEL === "prod" &&
...
const deepLinkEvent = "navi:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__NAVI__ ??= {}
  const pending = window.__NAVI__.deepLinks ?? []
  window.__NAVI__.deepLinks = [...pending, ...urls]
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://navi.ai/favicon-96x96-v3.png",
      })
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("navi.global.dat").getItem("language")
