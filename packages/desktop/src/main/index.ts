...
const APP_IDS: Record<string, string> = {
  dev: "ai.navi.desktop.dev",
  beta: "ai.navi.desktop.beta",
  prod: "ai.navi.desktop",
}
const TEST_ONBOARDING = process.env.NAVI_TEST_ONBOARDING === "1"
...
const main = Effect.gen(function* () {
  ...
  process.env.NAVI_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.navi.desktop.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `navi-onboarding-${randomUUID()}`)
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "Navi Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("navi://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("navi")
  registerRendererProtocol()
  setDockIcon()
  setupAutoUpdater()

  const needsMigration = ((): boolean => {
    if (process.env.NAVI_DB === ":memory:") return false

    const xdg = process.env.XDG_DATA_HOME
    const base = xdg && xdg.length > 0 ? xdg : join(homedir(), ".local", "share")
    return !existsSync(join(base, "navi", "navi.db"))
  })()
  let overlay: BrowserWindow | null = null

  const port = yield* Effect.gen(function* () {
    const fromEnv = process.env.NAVI_PORT
    if (fromEnv) {
