...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.navi.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `navi.settings.dat` is special: it maps to the `navi.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"navi.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // navi.settings.dat → the electron settings store ("navi.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "navi.global.dat").
  const storeName = filename === "navi.settings.dat" ? "navi.settings" : filename
