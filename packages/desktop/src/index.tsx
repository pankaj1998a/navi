...
import { AppBaseProviders, AppInterface, PlatformProvider, Platform } from "@navi-ai/app"
...
const createPlatform = (password: Accessor<string | null>): Platform => ({
    ...
    await Promise.resolve()
      .then(() => {
        const notification = new Notification(title, {
          body: description ?? "",
          icon: "https://navi.ai/favicon-96x96.png",
        })
  ...
  fetch: (input, init) => {
    ...
    const addHeader = (headers: Headers, password: string) => {
      headers.append("Authorization", `Basic ${btoa(`navi:${password}`)}`)
    }
      ...
      <AppBaseProviders>
        ...
        <ServerGate>
          {(data) => {
            ...
            window.__navi__ ??= {}
            window.__navi__.serverPassword = data().password ?? undefined
