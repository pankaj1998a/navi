...
const SETTINGS_STORE: &str = "navi.settings.dat";
...
fn get_sidecar_port() -> u32 {
    option_env!("navi_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("navi_PORT").ok())
...
fn spawn_sidecar(app: &AppHandle, port: u32, password: &str) -> CommandChild {
    ...
    let (mut rx, child) = cli::create_command(app, format!("serve --port {port}").as_str())
        .env("navi_SERVER_PASSWORD", password)
        .spawn()
        .expect("Failed to spawn navi");
...
async fn check_server_health(url: &str, password: Option<&str>) -> bool {
    ...
    if let Some(password) = password {
        req = req.basic_auth("navi", Some(password));
    }
            ...
            let mut window_builder =
                WebviewWindow::builder(&app, "main", WebviewUrl::App("/".into()))
                    ...
                    .initialization_script(format!(
                        r#"
                      window.__navi__ ??= {{}};
                      window.__navi__.updaterEnabled = {updater_enabled};
                    "#
                    ));
...
async fn spawn_local_server(
    ...
    loop {
        if timestamp.elapsed() > Duration::from_secs(30) {
            break Err(format!(
                "Failed to spawn navi Server. Logs:\n{}",
