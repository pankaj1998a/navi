...
import { Avatar } from "@navi-ai/ui/avatar"
...
import { Tooltip, TooltipKeybind } from "@navi-ai/ui/tooltip"
import { Collapsible } from "@navi-ai/ui/collapsible"
import { DiffChanges } from "@navi-ai/ui/diff-changes"
...
import { showToast, Toast, toaster } from "@navi-ai/ui/toast"
import { useGlobalSDK } from "@/context/global-sdk"
import { useNotification } from "@/context/notification"
import { usePermission } from "@/context/permission"
import { Binary } from "@navi-ai/util/binary"
import { retry } from "@navi-ai/util/retry"

import { useDialog } from "@navi-ai/ui/context/dialog"
import { useTheme, type ColorScheme } from "@navi-ai/ui/theme"
...
export default function Layout(props: ParentProps) {
    ...
    async function pollUpdate() {
      ...
      if (updateAvailable && toastId === undefined) {
        toastId = showToast({
          ...
          description: `A new version of Navi (${version}) is now available to install.`,
          actions: [
            {
  ...
  const ProjectAvatar = (props: {
    ...
    const navi = "4b0ea68d7af9a6031a7ffda7ad66e0cb83315750"

    return (
      <div class="relative size-5 shrink-0 rounded-sm">
        <Avatar
          fallback={name()}
          src={props.project.id === navi ? "https://navi.ai/favicon.svg" : props.project.icon?.url}
      ...
      <div class="flex flex-col self-stretch h-full items-center justify-between overflow-hidden min-h-0">
        ...
        <div class="flex flex-col gap-1.5 self-stretch items-start shrink-0 px-2 py-3">
            ...
            <Match when={providers.all().length > 0 && !providers.paid().length && expanded()}>
              <div class="rounded-md bg-background-stronger shadow-xs-border-base">
                <div class="p-3 flex flex-col gap-2">
                  ...
                  <div class="text-text-base">Navi includes free models so you can start immediately.</div>
          ...
          <Tooltip placement="right" value="Share feedback" inactive={expanded()}>
            <Button
              as={"a"}
              href="https://navi.ai/desktop-feedback"
              target="_blank"
              class="flex w-full text-left justify-start text-text-base stroke-[1.5px] rounded-lg px-2"
              variant="ghost"
              size="large"
