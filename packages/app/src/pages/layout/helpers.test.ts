...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("navi://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("navi://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("navi://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("navi://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("navi://open-project")).toBeUndefined()
    expect(parseDeepLink("navi://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "navi://open-project?directory=/a",
      "navi://other?directory=/b",
      "navi://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("navi://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("navi://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("navi://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("navi://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "navi://new-session?directory=/a",
      "navi://open-project?directory=/b",
      "navi://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __NAVI__: {
        deepLinks: ["navi://open-project?directory=/a"],
      },
    } as unknown as Window & { __NAVI__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["navi://open-project?directory=/a"])
