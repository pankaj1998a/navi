...
function formatInitError(error: InitError): string {
  ...
  switch (error.name) {
    case "MCPFailed":
      return `MCP server "${data.name}" failed. Note, navi does not support MCP authentication yet.`
    ...
    case "ProviderModelNotFoundError": {
      ...
      return [
        ...
        `Check your config (navi.json) provider/model names`,
      ].join("\n")
    }
      ...
      <div class="w-2/3 max-w-3xl flex flex-col items-center justify-center gap-8">
        ...
        <div class="flex flex-col items-center gap-2">
          <div class="flex items-center justify-center gap-1">
            Please report this error to the Navi team
            <button
              type="button"
              class="flex items-center text-text-interactive-base gap-1"
              onClick={() => platform.openLink("https://navi.ai/desktop-feedback")}
            >
              <div>on Discord</div>
