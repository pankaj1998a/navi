...
import { HookEventHandler } from './hookEventHandler.ts';
import type { Config } from '../config/config.ts';
import type { HookConfig } from './types.ts';
import type { Logger } from '@opentelemetry/api-logs';
import type { HookPlanner } from './hookPlanner.ts';
import type { HookRunner } from './hookRunner.ts';
import type { HookAggregator } from './hookAggregator.ts';
import { HookEventName, HookType } from './types.ts';
import {
  NotificationType,
  SessionStartSource,
...
vi.mock('../util/events.ts', () => ({
  coreEvents: mockCoreEvents,
}));

vi.mock('../telemetry/clearcut-logger/clearcut-logger.ts', () => ({
  ClearcutLogger: {
      ...
      getGeminiClient: vi.fn().mockReturnValue({
        getChatRecordingService: vi.fn().mockReturnValue({
          getConversationFilePath: vi
            .fn()
            .mockReturnValue('/test/project/.gemini/tmp/chats/session.tson'),
        }),
      }),
  ...
  describe('createBaseInput', () => {
    it('should create base input with correct fields', async () => {
      ...
      expect(mockHookRunner.executeHooksParallel).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          session_id: 'test-session',
          transcript_path: '/test/project/.gemini/tmp/chats/session.tson',
          cwd: '/test/project',
          hook_event_name: 'BeforeTool',
