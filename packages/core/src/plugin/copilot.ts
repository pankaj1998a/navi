...
const log = Log.create({ service: "copilot-plugin" })
  ...
  return {
          ...
          async fetch(request: RequestInfo | URL, init?: RequestInit) {
            ...
            const { isVision, isAgent } = iife(() => {
              try {
                ...
                log.debug("Failed to parse request body in Copilot plugin", { error: err, url })
              }
    ...
    "chat.headers": async (incoming, output) => {
      ...
      const parts = await sdk.session
        ...
        .catch((err) => {
          log.debug("Failed to fetch session message in Copilot plugin", { error: err })
          return undefined
        })
      ...
      const session = await sdk.session
        ...
        .catch((err) => {
          log.debug("Failed to fetch session in Copilot plugin", { error: err })
          return undefined
        })
