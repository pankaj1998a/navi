...
export interface TuiPluginMeta {
  ...
  source: string
  target: string
  state?: "first" | "updated" | "same" | "installed" | "removed"
}
...
export interface TuiPluginApi {
  ...
  ui: {
    Dialog(props: { size?: "sm" | "md" | "lg" | "full" | "medium" | "large" | "xlarge"; onClose?: () => void; children: JSX.Element }): JSX.Element
    ...
    DialogPrompt(props: { title: string; description?: () => JSX.Element; placeholder?: string; busy?: boolean; busyText?: string; onConfirm: (value: string) => void; onCancel?: () => void }): JSX.Element
    ...
    dialog: {
      ...
      clear(): void
      setSize(size: "sm" | "md" | "lg" | "full" | "medium" | "large" | "xlarge"): void
      readonly size: "sm" | "md" | "lg" | "full" | "medium" | "large" | "xlarge"
  ...
  slots: {
    register(plugin: { id?: string; order?: number; slots: Record<string, (ctx: TuiSlotContext, props: any) => JSX.Element> }): string
  }
  plugins: {
    ...
    install(spec: string, options?: { global?: boolean }): Promise<TuiPluginInstallResult>
  }
  lifecycle: {
