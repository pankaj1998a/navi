...
import { IdeClient, IDEConnectionStatus } from './ide-client.ts';
import * as fs from 'node:fs';
import { getIdeProcessInfo } from './process-utils.ts';
...
import { detectIde, IDE_DEFINITIONS } from './detect-ide.ts';
...
vi.mock('./process-utils.ts');
vi.mock('@modelcontextprotocol/sdk/client/index.ts');
vi.mock('@modelcontextprotocol/sdk/client/streamableHttp.ts');
vi.mock('@modelcontextprotocol/sdk/client/stdio.ts');
vi.mock('./detect-ide.ts');
vi.mock('node:os');
  ...
  describe('connect', () => {
    it('should connect using HTTP when port is provided in config file', async () => {
      ...
      expect(fs.promises.readFile).toHaveBeenCalledWith(
        path.join('/tmp/', 'gemini-ide-server-12345.tson'),
        'utf8',
      );
  ...
  describe('getConnectionConfigFromFile', () => {
    ...
    it('should find and parse a single config file with the new naming scheme', async () => {
      ...
      (
        vi.mocked(fs.promises.readdir) as Mock<
          (path: fs.PathLike) => Promise<string[]>
        >
      ).mockResolvedValue(['gemini-ide-server-12345-123.tson']);
      vi.mocked(fs.promises.readFile).mockResolvedValue(JSON.stringify(config));
      ...
      expect(result).toEqual(config);
      expect(fs.promises.readFile).toHaveBeenCalledWith(
        path.join('/tmp/gemini/ide', 'gemini-ide-server-12345-123.tson'),
        'utf8',
      );
    });
    ...
    it('should filter out configs with invalid workspace paths', async () => {
      ...
      (
        ...
        'gemini-ide-server-12345-111.tson',
        'gemini-ide-server-12345-222.tson',
      ]);
    ...
    it('should ignore files with invalid names', async () => {
      ...
      (
        ...
        'gemini-ide-server-12345-111.tson', // valid
        'not-a-config-file.txt', // invalid
        'gemini-ide-server-asdf.tson', // invalid
      ]);
      ...
      expect(result).toEqual(validConfig);
      expect(fs.promises.readFile).toHaveBeenCalledWith(
        path.join('/tmp/gemini/ide', 'gemini-ide-server-12345-111.tson'),
        'utf8',
      );
