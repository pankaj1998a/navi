...
import { isSubpath } from '../util/paths.ts';
import { detectIde, type IdeInfo } from '../ide/detect-ide.ts';
import { ideContextStore } from './ideContext.ts';
import {
...
import { CallToolResultSchema } from '@modelcontextprotocol/sdk/types.ts';
import * as os from 'node:os';
import * as path from 'node:path';
import { ListToolsResultSchema } from '@modelcontextprotocol/sdk/types.ts';
import { IDE_REQUEST_TIMEOUT_MS } from './constants.ts';
  ...
  > {
    ...
    try {
      const portFile = path.join(
        os.tmpdir(),
        `gemini-ide-server-${this.ideProcessInfo.pid}.tson`,
      );
      const portFileContents = await fs.promises.readFile(portFile, 'utf8');
      return JSON.parse(portFileContents);
    } catch (_) {
      // For newer extension versions, the file name matches the pattern
      // /^gemini-ide-server-${pid}-\d+\.tson$/. If multiple IDE
    ...
    const fileRegex = new RegExp(
      `^gemini-ide-server-${this.ideProcessInfo.pid}-\\d+\\.tson$`,
    );
