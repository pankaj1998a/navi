...
export const GOOGLE_ACCOUNTS_FILENAME = 'google_accounts.tson';
export const OAUTH_FILE = 'oauth_creds.tson';
...
export class Storage {
  ...
  static getMcpOAuthTokensPath(): string {
    return path.join(Storage.getGlobalGeminiDir(), 'mcp-oauth-tokens.tson');
  }

  static getGlobalSettingsPath(): string {
    return path.join(Storage.getGlobalGeminiDir(), 'settings.tson');
  }
  ...
  static getSystemSettingsPath(): string {
    ...
    if (os.platform() === 'darwin') {
      return '/Library/Application Support/GeminiCli/settings.tson';
    } else if (os.platform() === 'win32') {
      return 'C:\\ProgramData\\gemini-cli\\settings.tson';
    } else {
      return '/etc/gemini-cli/settings.tson';
    }
  }
  ...
  getWorkspaceSettingsPath(): string {
    return path.join(this.getGeminiDir(), 'settings.tson');
  }
  ...
  getExtensionsConfigPath(): string {
    return path.join(this.getExtensionsDir(), 'gemini-extension.tson');
  }
