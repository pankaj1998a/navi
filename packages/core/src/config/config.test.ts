...
import type { ConfigParameters, SandboxConfig } from './config.ts';
import { Config, DEFAULT_FILE_FILTERING_OPTIONS } from './config.ts';
...
import { HookType, HookEventName } from '../hooks/types.ts';
import * as path from 'node:path';
import { setGeminiMdFilename as mockSetGeminiMdFilename } from '../tools/memoryTool.ts';
import {
  DEFAULT_TELEMETRY_TARGET,
  DEFAULT_OTLP_ENDPOINT,
} from '../telemetry/index.ts';
import type { ContentGeneratorConfig } from '../core/contentGenerator.ts';
import {
  AuthType,
...
import { RipGrepTool, canUseRipgrep } from '../tools/ripGrep.ts';
import { logRipgrepFallback } from '../telemetry/loggers.ts';
import { RipgrepFallbackEvent } from '../telemetry/types.ts';
import { ToolRegistry } from '../tools/tool-registry.ts';
import { ACTIVATE_SKILL_TOOL_NAME } from '../tools/tool-names.ts';
...
vi.mock('../tools/mcp-client-manager.ts', () => ({
...
vi.mock('../util/memoryDiscovery.ts', () => ({
...
vi.mock('../tools/grep.ts');
vi.mock('../tools/ripGrep.ts', () => ({
  canUseRipgrep: vi.fn(),
...
vi.mock('../core/contentGenerator.ts');
...
vi.mock('../telemetry/index.ts', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../telemetry/index.ts')>();
  return {
    ...actual,
...
vi.mock('../telemetry/loggers.ts', async (importOriginal) => {
  const actual =
    await importOriginal<typeof import('../telemetry/loggers.ts')>();
  return {
    ...actual,
...
vi.mock('../services/gitService.ts', () => {
  const GitServiceMock = vi.fn();
  GitServiceMock.prototype.initialize = vi.fn();
  return { GitService: GitServiceMock };
});

vi.mock('../ide/ide-client.ts', () => ({
  IdeClient: {
...
vi.mock('../agents/delegate-to-agent-tool.ts', () => ({
  DelegateToAgentTool: vi.fn(),
}));

vi.mock('../resources/resource-registry.ts', () => ({
...
vi.mock('../util/events.ts', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../util/events.ts')>();
  return {
    ...actual,
...
vi.mock('../util/fetch.ts', () => ({
  setGlobalProxy: mockSetGlobalProxy,
}));

vi.mock('../services/contextManager.ts');

import { BaseLlmClient } from '../core/baseLlmClient.ts';
import { tokenLimit } from '../core/tokenLimits.ts';
import { uiTelemetryService } from '../telemetry/index.ts';
...
import type { CodeAssistServer } from '../code_assist/server.ts';
import { ContextManager } from '../services/contextManager.ts';

vi.mock('../core/baseLlmClient.ts');
vi.mock('../core/tokenLimits.ts', () => ({
  tokenLimit: vi.fn(),
}));
vi.mock('../code_assist/codeAssist.ts');
vi.mock('../code_assist/experiments/experiments.ts');
  ...
  describe('createToolRegistry', () => {
    ...
    it('should register subagents as tools when codebaseInvestigatorSettings.enabled is true', async () => {
      ...
      const AgentRegistryMock = (
        (await vi.importMock('../agents/registry.ts')) as {
          AgentRegistry: Mock;
        }
      ).AgentRegistry;
      ...
      const DelegateToAgentToolMock = (
        (await vi.importMock('../agents/delegate-to-agent-tool.ts')) as {
