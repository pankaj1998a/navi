...
import { FileDiscoveryService } from './fileDiscoveryService.ts';
...
describe('FileDiscoveryService', () => {
  ...
  describe('initialization', () => {
    it('should initialize git ignore parser by default in a git repo', async () => {
      ...
      expect(service.shouldIgnoreFile('node_modules/foo.ts')).toBe(true);
      expect(service.shouldIgnoreFile('src/foo.ts')).toBe(false);
    });
    ...
    it('should not load git repo patterns when not in a git repo', async () => {
      ...
      expect(service.shouldIgnoreFile('node_modules/foo.ts')).toBe(false);
    });
  ...
  describe('filterFiles', () => {
    ...
    it('should not filter files when respectGitIgnore is false', () => {
      ...
      expect(filtered).toEqual(
        ['src/index.ts', 'node_modules/package/index.ts', '.git/config'].map(
    ...
    it('should return true for git-ignored files', () => {
      ...
      expect(
        service.shouldIgnoreFile(
          path.join(projectRoot, 'node_modules/package/index.ts'),
        ),
      ).toBe(true);
    });
  ...
  describe('precedence (.geminiignore over .gitignore)', () => {
    ...
    it('should use .geminiignore rules if respectGitIgnore is false', async () => {
      ...
      const service = new FileDiscoveryService(projectRoot);
      const files = ['file.txt', 'important.txt', 'temp/file.ts'].map((f) =>
        path.join(projectRoot, f),
      );
