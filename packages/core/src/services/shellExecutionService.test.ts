...
} from './shellExecutionService.ts';
import { ShellExecutionService } from './shellExecutionService.ts';
import type { AnsiOutput, AnsiToken } from '../util/terminalSerializer.ts';
...
vi.mock('../util/shell-utils.ts', async (importOriginal) => {
  const actual =
    await importOriginal<typeof import('../util/shell-utils.ts')>();
  return {
    ...actual,
...
vi.mock('../util/getPty.ts', () => ({
  getPty: mockGetPty,
}));
vi.mock('../util/terminalSerializer.ts', () => ({
  serializeTerminalToObject: mockSerializeTerminalToObject,
}));
vi.mock('../util/systemEncoding.ts', () => ({
...
describe('ShellExecutionService', () => {
  ...
  describe('Successful Execution', () => {
    ...
    it('should truncate output exceeding the scrollback limit', async () => {
      ...
      // xterm.ts scrollback is the number of lines *above* the viewport.
