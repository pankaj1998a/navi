...
import { SessionSummaryService } from './sessionSummaryService.ts';
...
export async function getPreviousSession(
  ...
  try {
    ...
    const sessionFiles = allFiles.filter(
      (f) => f.startsWith(SESSION_FILE_PREFIX) && f.endsWith('.tson'),
    );
    ...
    // Filename format: session-YYYY-MM-DDTHH-MM-XXXXXXXX.tson
