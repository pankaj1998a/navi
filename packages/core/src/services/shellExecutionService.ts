...
import type { PtyImplementation } from '../util/getPty.ts';
import { getPty } from '../util/getPty.ts';
import { spawn as cpSpawn } from 'node:child_process';
import { TextDecoder } from 'node:util';
import os from 'node:os';
import { getCachedEncodingForBuffer } from '../util/systemEncoding.ts';
...
import { isBinary } from '../util/textUtils.ts';
import pkg from '@xterm/headless';
import {
  serializeTerminalToObject,
  type AnsiOutput,
} from '../util/terminalSerializer.ts';
import {
  sanitizeEnvironment,
  type EnvironmentSanitizationConfig,
} from './environmentSanitization.ts';
const { Terminal } = pkg;
