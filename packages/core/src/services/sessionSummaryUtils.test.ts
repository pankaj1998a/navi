...
import { generateSummary, getPreviousSession } from './sessionSummaryUtils.ts';
...
vi.mock('./sessionSummaryService.ts', () => ({
...
describe('sessionSummaryUtils', () => {
  ...
  describe('getPreviousSession', () => {
    ...
    it('should return null if most recent session already has summary', async () => {
      vi.mocked(fs.access).mockResolvedValue(undefined);
      mockReaddir.mockResolvedValue(['session-2024-01-01T10-00-abc12345.tson']);
    ...
    it('should return path if most recent session has more than 1 user message and no summary', async () => {
      ...
      expect(result).toBe(
        path.join(
          '/tmp/project',
          'chats',
          'session-2024-01-01T10-00-abc12345.tson',
        ),
      );
    });
    ...
    it('should select most recently created session by filename', async () => {
      vi.mocked(fs.access).mockResolvedValue(undefined);
      mockReaddir.mockResolvedValue([
        'session-2024-01-01T10-00-older000.tson',
        'session-2024-01-02T10-00-newer000.tson',
      ]);
      vi.mocked(fs.readFile).mockResolvedValue(
        createSessionWithUserMessages(2),
      );
