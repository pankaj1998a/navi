...
import { FORCE_ENCRYPTED_FILE_ENV_VAR } from '../mcp/token-storage/index.ts';
import { GEMINI_DIR, homedir as pathsHomedir } from '../util/paths.ts';
import { debugLogger } from '../util/debugLogger.ts';
import { writeToStdout } from '../util/stdio.ts';
import { FatalCancellationError } from '../util/errors.ts';
...
vi.mock('../util/paths.ts', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../util/paths.ts')>();
  return {
    ...actual,
    homedir: vi.fn(),
  };
...
vi.mock('../util/browser.ts', () => ({
  shouldAttemptBrowserLaunch: () => true,
}));
vi.mock('../util/stdio.ts', () => ({
  writeToStdout: vi.fn(),
  writeToStderr: vi.fn(),
...
vi.mock('./oauth-credential-storage.ts', () => ({
...
describe('oauth2', () => {
  describe('with encrypted flag false', () => {
    ...
    it('should clear credentials file', async () => {
      ...
      const credsPath = path.join(tempHomeDir, GEMINI_DIR, 'oauth_creds.tson');
    ...
    describe('credential loading order', () => {
      it('should prioritize default cached credentials over GOOGLE_APPLICATION_CREDENTIALS', async () => {
        ...
        const envCredsPath = path.join(tempHomeDir, 'env_creds.tson');
      ...
      it('should use GoogleAuth for BYOID credentials from GOOGLE_APPLICATION_CREDENTIALS', async () => {
        ...
        const envCredsPath = path.join(tempHomeDir, 'byoid_creds.tson');
        await fs.promises.writeFile(
          envCredsPath,
