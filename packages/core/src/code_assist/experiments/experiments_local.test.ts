...
import type { CodeAssistServer } from '../server.ts';
import * as fs from 'node:fs';
import * as os from 'node:os';
import type { ListExperimentsResponse } from './types.ts';
...
describe('experiments with GEMINI_EXP', () => {
  ...
  it('should read experiments from local file if GEMINI_EXP is set', async () => {
    process.env['GEMINI_EXP'] = '/tmp/experiments.tson';
    ...
    const { getExperiments } = await import('./experiments.ts');
    const experiments = await getExperiments(mockServer);

    expect(fs.promises.readFile).toHaveBeenCalledWith(
      '/tmp/experiments.tson',
      'utf8',
    );
  ...
  it('should fall back to server if reading file fails', async () => {
    process.env['GEMINI_EXP'] = '/tmp/missing.tson';
    ...
    const { getClientMetadata } = await import('./client_metadata.ts');
  ...
  it('should fallback to server if file has invalid structure', async () => {
    process.env['GEMINI_EXP'] = '/tmp/invalid.tson';
    const mockFileContent = JSON.stringify({
