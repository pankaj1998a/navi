...
import { ReleaseChannel, getReleaseChannel } from '../../util/channel.ts';
import { getVersion } from '../../util/version.ts';

// Mock dependencies before importing the module under test
vi.mock('../../util/channel.ts', async () => {
  const actual = await vi.importActual('../../util/channel.ts');
  return {
    ...(actual as object),
...
describe('client_metadata', () => {
  ...
  beforeEach(async () => {
    ...
    await import('./client_metadata.ts');
