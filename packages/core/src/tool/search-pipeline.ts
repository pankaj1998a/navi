...
export type SearchProvider = "browser" | "google" | "bing" | "duckduckgo" | "tavily" | "firecrawl" | "exa" | "brave" | "searxng"
...
const DEFAULT_PROVIDER_ORDER: SearchProvider[] = ["browser", "tavily", "exa", "brave", "firecrawl", "google", "bing", "duckduckgo", "searxng"]
...
export function resolveProviderOrder(preferred?: SearchProvider[]): SearchProvider[] {
  ...
  const parsed = raw
    ...
    .filter(Boolean)
      .filter((item): item is SearchProvider => ["browser", "google", "bing", "duckduckgo", "tavily", "firecrawl", "exa", "brave", "searxng"].includes(item))
