...
const log = Log.create({ service: "google-search" })
    ...
    async execute(params, ctx) {
        ...
        if (googleApiUrl) {
            try {
                ...
                log.warn("Custom Google Search API failed", { error: err, url: googleApiUrl })
            }
        }
...
async function summarizeWithGoogleAi(
    ...
    try {
        ...
        log.warn("summarizeWithGoogleAi failed, falling back to heuristic", { error: err })
        return { mode: "heuristic", summary: summarizeResults(results) }
    }
}
