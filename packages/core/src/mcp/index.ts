...
export namespace MCP {
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@navi/MCP") {}
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const connectTransport = (transport: Transport, timeout: number) =>
        Effect.acquireUseRelease(
          ...
          (t) =>
            Effect.tryPromise({
              try: () => {
                const client = new Client({ name: "Navi", version: Installation.VERSION })
              ...
              if (isAuthError) {
                ...
                if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
                  ...
                  return bus
                    .publish(TuiEvent.ToastShow, {
                      title: "MCP Authentication Required",
                      message: `Server "${key}" requires authentication. Run: Navi mcp auth ${key}`,
                      variant: "warning",
                      duration: 8000,
                    })
                    .pipe(Effect.ignore, Effect.as(undefined))
      ...
      const create = Effect.fn("MCP.create")(function* (key: string, mcp: Config.Mcp) {
        ...
        if (!listed) {
          yield* Effect.tryPromise(() => mcpClient.close()).pipe(
            Effect.catch((err) => {
              log.debug("Failed to close MCP client after tool list failure", { key, error: err })
              return Effect.void
            }),
          )
                    ...
                    if (typeof pid === "number") {
                      ...
                      for (const dpid of pids) {
                        try {
                          ...
                          log.debug("Failed to kill descendant process", { pid: dpid, parentPid: pid, error: err })
                        }
                      }
                    }
                    yield* Effect.tryPromise(() => client.close()).pipe(
                      Effect.catch((err) => {
                        log.debug("Failed to close MCP client during cleanup", { error: err })
                        return Effect.void
                      }),
                    )
                  }),
      ...
      function closeClient(s: State, name: string) {
        ...
        return Effect.tryPromise(() => client.close()).pipe(
          Effect.catch((err) => {
            log.debug("Failed to close MCP client", { name, error: err })
            return Effect.void
          }),
        )
      }
