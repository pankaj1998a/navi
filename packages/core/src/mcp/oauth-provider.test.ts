...
vi.mock('node:crypto');
vi.mock('./oauth-token-storage.ts', () => {
...
import type { OAuthToken } from './token-storage/types.ts';
import { MCPOAuthTokenStorage } from './oauth-token-storage.ts';
import {
  OAuthUtils,
...
const createMockResponse = (options: {
  ...
  if (options.tson !== undefined) {
    response.tson =
      typeof options.tson === 'function'
        ? (options.tson as () => Promise<unknown>)
        : () => Promise.resolve(options.tson);
  }
