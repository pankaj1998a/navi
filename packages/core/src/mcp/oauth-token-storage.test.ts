...
import { FORCE_ENCRYPTED_FILE_ENV_VAR } from './token-storage/index.ts';
import type { OAuthCredentials, OAuthToken } from './token-storage/types.ts';
...
vi.mock('./token-storage/hybrid-token-storage.ts', () => ({
    ...
    describe('getAllCredentials', () => {
      ...
      it('should load tokens from file successfully', async () => {
        ...
        expect(fs.readFile).toHaveBeenCalledWith(
          path.join('/mock/home', GEMINI_DIR, 'mcp-oauth-tokens.tson'),
          'utf-8',
        );
      });
