...
import { loadPoliciesFromToml, type PolicyFileError } from './toml-loader.ts';
...
import { SHELL_TOOL_NAMES } from '../util/shell-utils.ts';
import { SHELL_TOOL_NAME } from '../tools/tool-names.ts';
    ...
    async (message: UpdatePolicy) => {
      ...
      if (message.persist) {
        try {
          ...
          const newContent = toml.stringify(existingData as toml.tsonMap);
