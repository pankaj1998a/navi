...
import type { PolicySettings } from './types.ts';
import { ApprovalMode, PolicyDecision, InProcessCheckerType } from './types.ts';
...
describe('createPolicyEngineConfig', () => {
  ...
  it('should return ASK_USER for write tools and ALLOW for read-only tools by default', async () => {
    ...
    vi.resetModules();
    const { createPolicyEngineConfig } = await import('./config.ts');
  ...
  it('should have YOLO allow-all rule beat write tool rules in YOLO mode', async () => {
    ...
    const { Storage: FreshStorage } = await import('../config/storage.ts');
