...
import { getCoreSystemPrompt, resolvePathFromEnv } from './prompts.ts';
...
vi.mock('../agents/codebase-investigator.ts', () => ({
  CodebaseInvestigatorAgent: { name: 'codebase_investigator' },
}));
vi.mock('../util/gitUtils', () => ({
  isGitRepository: vi.fn(),
}));
vi.mock('node:fs');
vi.mock('../config/models.ts', async (importOriginal) => {
