...
export class Logger {
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.geminiDir, `checkpoint-${encodedTag}.tson`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.geminiDir!, `checkpoint-${tag}.tson`);
    try {
