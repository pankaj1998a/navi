...
} from './contentGenerator.ts';
import { type GeminiChat } from './geminiChat.ts';
...
import { getCoreSystemPrompt } from './prompts.ts';
import { DEFAULT_GEMINI_MODEL_AUTO } from '../config/models.ts';
import { FileDiscoveryService } from '../services/fileDiscoveryService.ts';
import { setSimulate429 } from '../util/testUtils.ts';
import { tokenLimit } from './tokenLimits.ts';
import { ideContextStore } from '../ide/ideContext.ts';
import type { ModelRouterService } from '../routing/modelRouterService.ts';
import { uiTelemetryService } from '../telemetry/uiTelemetry.ts';
import { ChatCompressionService } from '../services/chatCompressionService.ts';
import type { ChatRecordingService } from '../services/chatRecordingService.ts';
import { createAvailabilityServiceMock } from '../availability/testUtils.ts';
import type { ModelAvailabilityService } from '../availability/modelAvailabilityService.ts';
import type {
  ModelConfigKey,
  ResolvedModelConfig,
} from '../services/modelConfigService.ts';
import { ClearcutLogger } from '../telemetry/clearcut-logger/clearcut-logger.ts';
import * as policyCatalog from '../availability/policyCatalog.ts';
import { partToString } from '../util/partUtils.ts';

vi.mock('../services/chatCompressionService.ts');
...
vi.mock('./turn', async (importOriginal) => {
  const actual = await importOriginal<typeof import('./turn.ts')>();
...
vi.mock('./prompts');
vi.mock('../util/getFolderStructure', () => ({
  getFolderStructure: vi.fn().mockResolvedValue('Mock Folder Structure'),
}));
vi.mock('../util/errorReporting', () => ({ reportError: vi.fn() }));
vi.mock('../util/nextSpeakerChecker', () => ({
  checkNextSpeaker: vi.fn().mockResolvedValue(null),
}));
vi.mock('../util/generateContentResponseUtilities', () => ({
  getResponseText: (result: GenerateContentResponse) =>
    result.candidates?.[0]?.content?.parts?.map((part) => part.text).join('') ||
    undefined,
}));
vi.mock('../telemetry/index.ts', () => ({
  logApiRequest: vi.fn(),
  logApiResponse: vi.fn(),
  logApiError: vi.fn(),
}));
vi.mock('../ide/ideContext.ts');
vi.mock('../telemetry/uiTelemetry.ts', () => ({
  uiTelemetryService: {
    setLastPromptTokenCount: vi.fn(),
    getLastPromptTokenCount: vi.fn(),
  },
}));
vi.mock('../hooks/hookSystem.ts');
...
describe('Gemini Client (client.ts)', () => {
  ...
  describe('tryCompressChat', () => {
    ...
    it('should resume the session file when compression succeeds', async () => {
      ...
      const mockFilePath = '/tmp/session.tson';
...
\`\`\`
    ...
    it('should use getGlobalMemory for system instruction when JIT is enabled', async () => {
      ...
      const { getCoreSystemPrompt } = await import('./prompts.ts');
