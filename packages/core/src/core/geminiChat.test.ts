...
import { DEFAULT_THINKING_MODE } from '../config/models.ts';
import { AuthType } from './contentGenerator.ts';
import { TerminalQuotaError } from '../util/googleQuotaErrors.ts';
import { type RetryOptions } from '../util/retry.ts';
import { uiTelemetryService } from '../telemetry/uiTelemetry.ts';
import { HookSystem } from '../hooks/hookSystem.ts';
...
import * as policyHelpers from '../availability/policyHelpers.ts';
import { makeResolvedModelConfig } from '../services/modelConfigServiceTestUtils.ts';
import {
  fireBeforeModelHook,
  fireAfterModelHook,
  fireBeforeToolSelectionHook,
} from './geminiChatHookTriggers.ts';

// Mock hook triggers
vi.mock('./geminiChatHookTriggers.ts', () => ({
...
vi.mock('../util/retry.ts', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../util/retry.ts')>();
  return {
    ...actual,
