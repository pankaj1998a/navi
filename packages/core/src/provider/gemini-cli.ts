...
async function getGeminiAuth() {
    ...
    try {
        ...
        if (creds.access_token && creds.refresh_token) {
            ...
            await Auth.set("gemini-cli", fallbackAuth).catch((e: any) => log.error("Gemini CLI: Failed to save migrated auth", { error: String(e) }))
            return fallbackAuth
        }
    } catch (e: any) {
                    ...
                    if (buffer.length > 0) {
                        ...
                        if (line.startsWith("data: ")) {
                            ...
                            try {
                                ...
                                log.debug("Gemini CLI: Failed to parse SSE payload", { line, error: String(e) })
                            }
                        }
