...
import { mockControl } from '../__mocks__/fs/promises.ts';
import { ReadManyFilesTool } from './read-many-files.ts';
...
import { createMockMessageBus } from '../test-utils/mock-message-bus.ts';

vi.mock('glob', { spy: true });

vi.mock('mime', () => {
  const getType = (filename: string) => {
    if (filename.endsWith('.ts') || filename.endsWith('.ts')) {
      return 'text/plain';
    }
...
describe('ReadManyFilesTool', () => {
  ...
  describe('build', () => {
    ...
    it('should return an invocation for valid glob patterns within root', () => {
      const params = { include: ['*.txt', 'subdir/**/*.ts'] };
      const invocation = tool.build(params);
      expect(invocation).toBeDefined();
    });
  ...
  describe('execute', () => {
    ...
    it('should read multiple specified files', async () => {
      ...
      createFile('subdir/file2.ts', 'Content2');
      const params = { include: ['file1.txt', 'subdir/file2.ts'] };
      const invocation = tool.build(params);
      const result = await invocation.execute(new AbortController().signal);
      const content = result.llmContent as string[];
      const expectedPath1 = path.join(tempRootDir, 'file1.txt');
      const expectedPath2 = path.join(tempRootDir, 'subdir/file2.ts');
      expect(
        content.some((c) =>
    ...
    it('should handle glob patterns', async () => {
      ...
      expect(content.find((c) => c.includes('sub/data.tson'))).toBeUndefined();
      expect(result.returnDisplay).toContain(
        'Successfully read and concatenated content from **2 file(s)**',
      );
    });
    ...
    it('should use default excludes', async () => {
      createFile('node_modules/some-lib/index.ts', 'lib code');
      createFile('src/app.ts', 'app code');
      const params = { include: ['**/*.ts'] };
      const invocation = tool.build(params);
      const result = await invocation.execute(new AbortController().signal);
      const content = result.llmContent as string[];
      const expectedPath = path.join(tempRootDir, 'src/app.ts');
      expect(content).toEqual([
        `--- ${expectedPath} ---\n\napp code\n\n`,
        `\n--- End of content ---`,
      ]);
      expect(
        content.find((c) => c.includes('node_modules/some-lib/index.ts')),
      ).toBeUndefined();
      expect(result.returnDisplay).toContain(
        'Successfully read and concatenated content from **1 file(s)**',
      );
    ...
    it('should NOT use default excludes if useDefaultExcludes is false', async () => {
      createFile('node_modules/some-lib/index.ts', 'lib code');
      createFile('src/app.ts', 'app code');
      const params = { include: ['**/*.ts'], useDefaultExcludes: false };
      const invocation = tool.build(params);
      const result = await invocation.execute(new AbortController().signal);
      const content = result.llmContent as string[];
      ...
      const expectedPath2 = path.join(tempRootDir, 'src/app.ts');
      expect(
        content.some((c) =>
          c.includes(`--- ${expectedPath1} ---\n\nlib code\n\n`),
        ),
  ...
  describe('Batch Processing', () => {
    ...
    it('should process files in parallel', async () => {
      ...
      const detectFileTypeSpy = vi.spyOn(
        await import('../util/fileUtils.ts'),
        'detectFileType',
      );
