...
import { DEFAULT_GUI_EDITOR } from '../util/editor.ts';
...
vi.mock('../util/editor.ts', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../util/editor.ts')>();
  return {
    ...actual,
