...
import type { GlobToolParams, GlobPath } from './glob.ts';
import { GlobTool, sortFileEntries } from './glob.ts';
import { partListUnionToString } from '../core/geminiRequest.ts';
  ...
  describe('validateToolParams', () => {
    it.each([
      ...
      },
      {
        ...
        params: { pattern: '*.ts', dir_path: 'sub' },
        expected: null,
      },
      {
        name: 'should return null for valid parameters (pattern, dir_path, and case_sensitive)',
        params: { pattern: '*.ts', dir_path: 'sub', case_sensitive: true },
        expected: null,
      },
      {
