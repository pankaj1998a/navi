...
import { GeminiChat, StreamEventType } from '../core/geminiChat.ts';
...
import { executeToolCall } from '../core/nonInteractiveToolExecutor.ts';
import { ToolRegistry } from './tool-registry.ts';
import { CompressionStatus } from '../services/chatCompressionService.ts';
import type { ToolCallRequestInfo } from '../scheduler/types.ts';
import { ChatCompressionService } from '../services/chatCompressionService.ts';
import { getDirectoryContextString } from '../util/environmentContext.ts';
import { promptIdContext } from '../util/promptIdContext.ts';
import {
  logAgentStart,
  logAgentFinish,
  logRecoveryAttempt,
...
import { AgentTerminateMode } from '../agents/index.ts';
import { templateString } from '../util/template.ts';
import { DEFAULT_GEMINI_MODEL, isAutoModel } from '../config/models.ts';
import type { RoutingContext } from '../routing/types.ts';
import { parseThought } from '../util/thoughtUtils.ts';
import { type z } from 'zod';
import { zodToJsonSchema } from 'zod-to-json-schema';
import { debugLogger } from '../util/debugLogger.ts';
import { getModelConfigAlias } from '../agents/agent-registry.ts';
...
export class LocalAgentExecutor<TOutput extends z.ZodTypeAny> {
  ...
  private async processFunctionCalls(
    ...
    // Using Promise.allSettled to ensure all tools complete even if some fail
    const asyncResults = await Promise.allSettled(toolExecutionPromises);

    // Combine all response parts
    const toolResponseParts: Part[] = [...syncResponseParts];
    for (const result of asyncResults) {
      if (result.status === 'fulfilled' && result.value) {
        toolResponseParts.push(...result.value);
      } else if (result.status === 'rejected') {
        // Log the error but don't stop other tools
        debugLogger.warn(`[LocalAgentExecutor] Tool execution failed: ${result.reason}`);
      }
    }
