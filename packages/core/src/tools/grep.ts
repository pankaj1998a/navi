...
import type { FileExclusions } from '../util/ignorePatterns.ts';
    ...
    super(
      ...
      Kind.Search,
      {
        properties: {
          ...
          include: {
            description:
              "Optional: A glob pattern to filter which files are searched (e.g., '*.ts', '*.{ts,tsx}', 'src/**'). If omitted, searches all files (respecting potential global ignores).",
            type: 'string',
          },
        },
        required: ['pattern'],
