...
import type { ProcessedFileReadResult } from '../util/fileUtils.ts';
import {
  detectFileType,
...
import { getProgrammingLanguage } from '../telemetry/telemetry-utils.ts';
...
import { READ_MANY_FILES_TOOL_NAME } from './tool-names.ts';
 ...
 * It is intended to run in an environment with access to the local file system (e.g., a Node.ts backend).
 */
