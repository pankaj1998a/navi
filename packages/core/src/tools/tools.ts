...
} from '../confirmation-bus/types.ts';
import { classifyToolApproval } from './approval-classifier.ts';
...
> implements ToolInvocation<TParams, TResult>
{
  constructor(
    ...
    readonly _kind?: Kind,
    readonly _workspaceRoots?: readonly string[],
  ) {}
  ...
  protected getMessageBusDecision(
    ...
    // Attempt auto-approval based on security classification
    if (this._kind && this._toolName) {
      const classification = classifyToolApproval({
        toolName: this._toolName,
        kind: this._kind,
        args: this.params as Record<string, unknown>,
        workspaceRoots: this._workspaceRoots ?? [],
      });
