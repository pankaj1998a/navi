...
import type { GrepToolParams } from './grep.ts';
import { GrepTool } from './grep.ts';
import path from 'node:path';
import fs from 'node:fs/promises';
import os from 'node:os';
import type { Config } from '../config/config.ts';
import { createMockWorkspaceContext } from '../test-utils/mockWorkspaceContext.ts';
...
describe('GrepTool', () => {
  ...
  beforeEach(async () => {
    ...
    await fs.writeFile(
      path.join(tempRootDir, 'fileB.ts'),
  ...
  describe('execute', () => {
    ...
    it('should find matches with an include glob', async () => {
      const params: GrepToolParams = { pattern: 'hello', include: '*.ts' };
      const invocation = grepTool.build(params);
      const result = await invocation.execute(abortSignal);
      expect(result.llmContent).toContain(
        'Found 1 match for pattern "hello" in the workspace directory (filter: "*.ts"):',
      );
      expect(result.llmContent).toContain('File: fileB.ts');
      expect(result.llmContent).toContain(
    ...
    it('should find matches with an include glob and path', async () => {
      await fs.writeFile(
        path.join(tempRootDir, 'sub', 'another.ts'),
      ...
      const result = await invocation.execute(abortSignal);
      expect(result.llmContent).toContain(
        'Found 1 match for pattern "hello" in path "sub" (filter: "*.ts")',
      );
      expect(result.llmContent).toContain('File: another.ts');
  ...
  describe('multi-directory workspace', () => {
    it('should search across all workspace directories when no path is specified', async () => {
      ...
      await fs.writeFile(
        path.join(secondDir, 'another.ts'),
      ...
      expect(result.llmContent).toContain(
        `File: ${path.join(secondDirName, 'another.ts')}`,
      );
