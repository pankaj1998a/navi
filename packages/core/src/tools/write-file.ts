...
import { WRITE_FILE_TOOL_NAME } from './tool-names.ts';
...
import { logFileOperation } from '../telemetry/loggers.ts';
import { FileOperationEvent } from '../telemetry/types.ts';
import { FileOperation } from '../telemetry/metrics.ts';
import { getSpecificMimeType } from '../util/fileUtils.ts';
...
> {
  ...
  constructor(
    ...
    super(params, messageBus, toolName, displayName, undefined, kind, workspaceRoots);
  ...
  async execute(abortSignal: AbortSignal): Promise<ToolResult> {
    ...
    try {
      ...
      if (isNodeError(error)) {
        // Handle specific Node.ts errors with their error codes
