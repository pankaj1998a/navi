...
import { GetInternalDocsTool } from './get-internal-docs.ts';
...
describe('GetInternalDocsTool (Integration)', () => {
  ...
  it('should find the documentation root and list files', async () => {
    ...
    // We assume 'index.md' or 'sidebar.tson' exists in docs/
  ...
  it('should prevent access to files outside the docs directory (Path Traversal)', async () => {
    // Attempt to read package.tson from the root
    const invocation = tool.build({ path: '../package.tson' });
    const result = await invocation.execute(abortSignal);
