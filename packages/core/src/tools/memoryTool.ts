...
import type { ToolEditConfirmationDetails, ToolResult, ToolInvocation } from './tools.ts';
import {
  BaseDeclarativeTool,
  BaseToolInvocation,
  Kind,
...
import { DEFAULT_DIFF_OPTIONS } from './diffOptions.ts';
import { tildeifyPath } from '../util/paths.ts';
import type {
  ModifiableDeclarativeTool,
  ModifyContext,
} from './modifiable-tool.ts';
import { ToolErrorType } from './tool-error.ts';
import { MEMORY_TOOL_NAME } from './tool-names.ts';
...
> {
  ...
  constructor(
    ...
    super(params, messageBus, _toolName, _displayName, undefined, _kind, _workspaceRoots);
  }
...
export class MemoryTool
  ...
  protected createInvocation(
    ...
    return new MemoryToolInvocation(
      ...
      _kind,
      [Storage.getGlobalGeminiDir()], // Workspace roots for memory (global only)
    );
  }
