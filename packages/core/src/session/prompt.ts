...
import { recommendAgentMode, collectPromptTaskText } from "../agent/mode-advisor"
import { ToolLoopDetector } from "../agent/tool-loop-detection"
...
export namespace SessionPrompt {
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@navi/SessionPrompt") {}
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const title = Effect.fn("SessionPrompt.ensureTitle")(function* (input: {
        ...
        if (!ag) return
        const mdl = ag.model
          ? yield* provider.getModel(ag.model.providerID as ProviderID, ag.model.modelID as ModelID)
      ...
      const insertReminders = Effect.fn("SessionPrompt.insertReminders")(function* (input: {
        ...
        if (!Flag.NAVI_EXPERIMENTAL_PLAN_MODE) {
...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
      ...
      const insertMemories = Effect.fn("SessionPrompt.insertMemories")(function* (input: {
        messages: MessageV2.WithParts[]
        sessionID: SessionID
      }) {
        const lastUser = input.messages.findLast((m) => m.info.role === "user")
        if (!lastUser) return input.messages
        
        const query = lastUser.parts.filter(p => p.type === "text").map((p: any) => p.text).join("\n")
        const relevant = yield* memory.findRelevant(query)
        if (relevant.length === 0) return input.messages

        let text = "\n\n# Recalled Memories:\n"
        text += "The following information is from prior conversations. It may be stale; verify against current code before acting.\n\n"
        text += "<memories>\n"
        for (const m of relevant) {
          text += `<memory name="${m.name}" type="${m.type}">\n${m.content}\n</memory>\n`
        }
        text += "</memories>"
      ...
      const insertScratchpad = Effect.fn("SessionPrompt.insertScratchpad")(function* (input: {
        messages: MessageV2.WithParts[]
        sessionID: SessionID
        session: Session.Info
      }) {
        if (!input.session.scratchpad) return input.messages
        const lastUser = input.messages.findLast((m) => m.info.role === "user")
        if (!lastUser) return input.messages
              ...
              return Effect.runPromise(
                Effect.gen(function* () {
                  ...
                  const hookResult = yield* Effect.promise(() =>
                    import("../hook/registry").then(({ onPreToolUse }) =>
                      onPreToolUse(ctx.sessionID, cwd, item.id, args, ctx.agent),
                    ),
                  )
                  if (!hookResult.shouldContinue) {
                    const reason = hookResult.blockReason ?? `PreToolUse hook blocked execution of ${item.id}`
                  ...
                  const loopCheck = ToolLoopDetector.check(ctx.sessionID, item.id, args)
                  if (loopCheck.isLoop) {
                      return {
                  ...
                  try {
                    result = yield* Effect.promise(() => item.execute(args, ctx))
                  } catch (err) {
                    // Fire PostToolUseFailure hook
                    yield* Effect.promise(() =>
                      import("../hook/registry").then(({ onPostToolUseFailure }) =>
                        onPostToolUseFailure(ctx.sessionID, cwd, item.id, args, String(err), ctx.agent),
                      ),
                    )
                    throw err
                  }
                  ...
                  const truncated = yield* truncate.output(textOutput, {}, input.agent);
                  ...
                  yield* Effect.promise(() =>
                    import("../hook/registry").then(({ onPostToolUse }) =>
                      onPostToolUse(ctx.sessionID, cwd, item.id, args, result.output, ctx.agent),
                    ),
                  )
      ...
      const shellImpl = Effect.fn("SessionPrompt.shellImpl")(function* (input: ShellInput, signal: AbortSignal) {
        ...
        const userMsg: MessageV2.User = {
          ...
          role: "user",
          agent: input.agent,
          model: { providerID: model.providerID as ProviderID, modelID: model.modelID as ModelID },
        }
      ...
      const createUserMessage = Effect.fn("SessionPrompt.createUserMessage")(function* (input: PromptInput) {
        ...
        const full = yield* (!input.variant && ag.variant && same
          ? provider.getModel(model.providerID as ProviderID, model.modelID as ModelID).pipe(
        ...
        if (!parsed.success) {
          log.error("invalid user message before save", {
            ...
            messageID: info.id,
            agent: info.agent,
            model: info.role === 'user' ? info.model : { providerID: (info as MessageV2.Assistant).providerID, modelID: (info as MessageV2.Assistant).modelID },
      ...
      const runWaterfall = Effect.fn("SessionPrompt.runWaterfall")(function* (input: {
        sessionID: SessionID
        goal: string
        lastUser: MessageV2.User
        session: Session.Info
      }) {
        const { sessionID, goal, lastUser, session } = input
        const ctx = yield* InstanceState.context
        const orchestrator = new Orchestrator()
        const workflow = orchestrator.waterfallWorkflow(goal)
          ...
          Effect.gen(function* () {
            if (typeof result !== "string" && "taskId" in result) {
              ...
              const assistantMessage: MessageV2.Assistant = yield* sessions.updateMessage({
                ...
                sessionID,
                mode: result.taskId, 
                agent: result.taskId.includes("architect") ? "architect" : "coding",
                variant: lastUser.variant,
                path: { cwd: ctx.directory, root: ctx.worktree },
                cost: 0,
                tokens: { input: 0, output: 0, reasoning: 0, cache: { read: 0, write: 0 } },
                modelID: lastUser.model.modelID as ModelID,
                providerID: lastUser.model.providerID as ProviderID,
                time: { created: Date.now(), completed: Date.now() },
                finish: result.success ? "stop" : "error",
              })
      ...
      const runLoop: (sessionID: SessionID) => Effect.Effect<MessageV2.WithParts> = Effect.fn("SessionPrompt.run")(
        function* (sessionID: SessionID) {
          ...
          while (true) {
            ...
            if (step === 0) ToolLoopDetector.clear(sessionID)
            ...
            // Check for Waterfall Workflow trigger
            const taskText = collectPromptTaskText({ input: msgs[msgs.length - 1]?.parts.find(p => p.type === 'text')?.text || "" })
            const isForge = taskText.startsWith("FORGE:")
            const recommendation = recommendAgentMode({ task: taskText, currentAgent: lastUser.agent })

            if (isForge || recommendation.workflow === "waterfall") {
              const goal = isForge ? taskText.replace("FORGE:", "").trim() : taskText
              return yield* runWaterfall({ sessionID, goal, lastUser, session }).pipe(Effect.orDie)
            }
            ...
            if (
              ...
              const overflowModel = yield* provider.getModel(
                (lastUser.model ? lastUser.model.providerID : (lastFinished as MessageV2.Assistant).providerID) as ProviderID,
                (lastUser.model ? lastUser.model.modelID : (lastFinished as MessageV2.Assistant).modelID) as ModelID
              )
              if (yield* compaction.isOverflow({ tokens: lastFinished.tokens, model: overflowModel })) {
            ...
            msgs = yield* insertMemories({ messages: msgs, sessionID })
            msgs = yield* insertScratchpad({ messages: msgs, sessionID, session })
      ...
      const command = Effect.fn("SessionPrompt.command")(function* (input: CommandInput) {
        ...
        if (input.command === Command.Default.AGENT) {
          try {
            while (true) {
              const allAgents = yield* agents.list()
              const myAgents = allAgents.filter((a) => !a.hidden)
              ...
              const selection = yield* question
                .ask({
                  ...
                  questions: [
                    {
                      header: "Select Agent",
                      question: "Which sub-agent would you like to configure?",
                      options: [
                        ...myAgents.map((a) => ({ label: a.name, description: a.description || "Sub-agent" })),
                        { label: "Done", description: "Exit configuration wizard" },
                      ],
                      custom: false,
                    },
                  ],
                })
                .pipe(Effect.catchTag("QuestionRejectedError", () => Effect.succeed([["Done"]] as Question.Answer[])))

              const agentName = selection[0][0]
              if (agentName === "Done") break

              const providers = yield* provider.list()
              
              const allModels = Object.values(providers).flatMap((p) => 
              ...
              const modelSelection = yield* question
                .ask({
                  sessionID: input.sessionID,
                  questions: [
                    {
                      header: "Select Model",
                      question: `Choose a model for '${agentName}':`,
                      options: allModels,
                      custom: false,
                    },
                  ],
              ...
              const selectedModel = modelSelection[0][0]
              if (!selectedModel) continue

              // Save preferences
              const currentPrefs = loadPreferences()
              const updatedAgentModels = { ...currentPrefs.agentModels, [agentName]: selectedModel }
              updatePreferences({ agentModels: updatedAgentModels })
            }
          } catch (e: any) {
            log.error("Error during agent configuration", { error: e?.message ?? String(e) })
          }

          const finalMsg: MessageV2.Assistant = {
            id: MessageID.ascending(),
          ...
          const part: MessageV2.TextPart = {
            ...
            sessionID: input.sessionID,
            type: "text",
            text: "✅ **System Configuration Complete**\n\nAll sub-agent model assignments have been updated.",
            time: { start: Date.now(), end: Date.now() },
          }

          yield* Effect.promise(() =>
            SyncEvent.run(MessageV2.Event.Updated, { sessionID: input.sessionID, info: finalMsg }),
          )
          yield* Effect.promise(() => SyncEvent.run(MessageV2.Event.PartUpdated, { sessionID: input.sessionID, part, time: Date.now() }))

          return { info: finalMsg, parts: [part] }
        }
        ...
        const taskModel = yield* Effect.gen(function* () {
          ...
          if (cmd.agent) {
            ...
            if (cmdAgent?.model) return { 
              providerID: cmdAgent.model.providerID as ProviderID, 
              modelID: cmdAgent.model.modelID as ModelID 
            }
          }
          const last = yield* lastModel(input.sessionID)
          return { 
            providerID: last.providerID, 
            modelID: ((last as any).modelID || (last as any).id) as ModelID 
          }
        })

        yield* getModel(taskModel.providerID as ProviderID, taskModel.modelID as ModelID, input.sessionID)
          ...
          ? [
              {
                ...
                model: { providerID: taskModel.providerID as ProviderID, modelID: taskModel.modelID as ModelID },
        ...
        const result = yield* prompt({
          sessionID: input.sessionID,
          messageID: input.messageID,
          model: { providerID: userModel.providerID as ProviderID, modelID: ((userModel as any).modelID || (userModel as any).id) as ModelID },
          agent: userAgent,
          parts,
          variant: input.variant,
        })
