...
const FILES = [
  "AGENTS.md",
  ...(Flag.NAVI_DISABLE_CLAUDE_CODE_PROMPT ? [] : ["CLAUDE.md"]),
...
function globalFiles() {
  ...
  if (Flag.NAVI_CONFIG_DIR) {
    files.push(path.join(Flag.NAVI_CONFIG_DIR, "AGENTS.md"))
  }
...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.NAVI_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no NAVI_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.NAVI_CONFIG_DIR, Flag.NAVI_CONFIG_DIR).catch(() => [])
}
