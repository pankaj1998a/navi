...
import { loadSkillsFromDir, type SkillDefinition } from './skillLoader.ts';
import { coreEvents } from '../util/events.ts';
import { debugLogger } from '../util/debugLogger.ts';

vi.mock('./skillLoader.ts', async (importOriginal) => {
  const actual = await importOriginal<typeof import('./skillLoader.ts')>();
  return {
    ...actual,
