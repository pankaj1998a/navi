...
import { ModelRouterService } from './modelRouterService.ts';
import { Config } from '../config/config.ts';

import type { BaseLlmClient } from '../core/baseLlmClient.ts';
import type { RoutingContext, RoutingDecision } from './routingStrategy.ts';
import { DefaultStrategy } from './strategies/defaultStrategy.ts';
import { CompositeStrategy } from './strategies/compositeStrategy.ts';
import { FallbackStrategy } from './strategies/fallbackStrategy.ts';
import { OverrideStrategy } from './strategies/overrideStrategy.ts';
import { ClassifierStrategy } from './strategies/classifierStrategy.ts';
import { logModelRouting } from '../telemetry/loggers.ts';
import { ModelRoutingEvent } from '../telemetry/types.ts';

vi.mock('../config/config.ts');
vi.mock('../core/baseLlmClient.ts');
vi.mock('./strategies/defaultStrategy.ts');
vi.mock('./strategies/compositeStrategy.ts');
vi.mock('./strategies/fallbackStrategy.ts');
vi.mock('./strategies/overrideStrategy.ts');
vi.mock('./strategies/classifierStrategy.ts');
vi.mock('../telemetry/loggers.ts');
vi.mock('../telemetry/types.ts');
