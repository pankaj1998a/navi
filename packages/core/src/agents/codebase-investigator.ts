...
import type { LocalAgentDefinition } from './types.ts';
import {
  READ_FILE_TOOL_NAME,
  LS_TOOL_NAME,
  GREP_TOOL_NAME,
...
import { DEFAULT_GEMINI_MODEL } from '../index.ts'
import { z } from 'zod';
...
> = {
  ...
  toolConfig: {
    ...
    tools: [READ_FILE_TOOL_NAME, LS_TOOL_NAME, GREP_TOOL_NAME, GLOB_TOOL_NAME],
  },
...
\`\`\`json
{
  ...
  "ExplorationTrace": [
    ...
    "Read the file \`src/controllers/userController.ts\` to understand the function's logic.",
    "Used \`ls -R\` to look for related files, such as services or database models.",
    "Read \`src/services/userService.ts\` and \`src/models/User.ts\` to understand the data flow and how state is managed."
  ],
  "RelevantLocations": [
    {
