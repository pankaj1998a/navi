...
import { createOpencode, type ToolPart } from "@navi-ai/sdk"
...
const navi = await createOpencode({
  port: 0,
})
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new navi session...")
    const { client, server } = navi
    ...
    console.log("✅ Created navi session:", createResult.data.id)
  ...
  console.log("📝 Sending to navi:", message.text)
