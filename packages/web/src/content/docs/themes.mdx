...
With Navi you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Navi uses our own `Navi` theme.
...
Navi comes with several built-in themes.
...
- Want Navi to match their terminal's appearance
...
Navi supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/Navi/themes/*.json` or `$XDG_CONFIG_HOME/Navi/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.Navi/themes/*.json`
4. **Current working directory** - Defined in `./.Navi/themes/*.json`
