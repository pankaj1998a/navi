...
Med Navi kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Navi vårt eget `Navi`-tema.
...
Navi støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/Navi/themes/*.json` eller `$XDG_CONFIG_HOME/Navi/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.Navi/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.Navi/themes/*.json`
