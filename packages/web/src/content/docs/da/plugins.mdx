---
title: Plugins
description: Skriv dine egne plugins for at udvide Navi.
---

Plugins giver dig mulighed for at udvide Navi ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Navis standardadfærd.
...
- `.Navi/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/Navi/node_modules/`.
...
1. Global konfiguration (`~/.config/Navi/Navi.json`)
2. Projektkonfiguration (`Navi.json`)
3. Global plugin-mappe (`~/.config/Navi/plugins/`)
4. Projekt plugin bibliotek (`.Navi/plugins/`)
...
Navi kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En Navi SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide Navi.
...
:::note
Hvis du bruger Navi desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå Navi i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til Navi:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som Navi kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for Navi sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://Navi.ai/docs/sdk) for detaljer.
