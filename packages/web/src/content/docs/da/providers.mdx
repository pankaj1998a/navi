---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Navi.
---

import config from "../../../../config.mjs"
export const console = config.console

Navi bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Navi-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/Navi/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Navi-konfigurationen.
...
Navi Zen er en liste over modeller leveret af Navi-teamet, som er blevet testet og verificeret til at fungere godt med Navi. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Navi Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg Navi og gå til [Navi.ai/auth](https://Navi.ai/auth).
...
Det fungerer som alle andre udbydere i Navi og er helt valgfrit at bruge.

---

## Navi Go

Navi Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Navi-teamet, som er testet og verificeret til at fungere godt med Navi.

1. Kør kommandoen `/connect` i TUI, vælg `Navi Go`, og gå til [Navi.ai/auth](https://Navi.ai/zen).
...
Sådan bruger du Amazon Bedrock med Navi:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører Navi:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `Navi.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Navi understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere Navi til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at Navi skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem Navi-konfigurationen.
...
gemme tokenet i Navi auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Navi bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Navi
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`Navi.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [Navi-gitlab-auth](https://www.npmjs.com/package/Navi-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `Navi.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Navi:
...
Sådan bruger du Google Vertex AI med Navi:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem Navi, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [Navi-helicone-session](https://github.com/H2Shami/Navi-helicone-session) plugin for automatisk at logge hver Navi-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Navi-samtale opført som en separat session.
...
Du kan konfigurere Navi til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere Navi til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere Navi til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Navi. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/Navi) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Navi:
...
3. Kopier API-nøglen til brug i Navi.
...
6. **Vigtigt**: Før du bruger skymodeller i Navi, skal du hente modelinformationen lokalt:
...
Navi Zen er en liste over testede og verificerede modeller leveret af Navi-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Navi Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Navi Zen**.

   ```txt
   /connect
   ```

3. Indtast Navi API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem Navi-konfigurationen.
...
5. Du kan også tilpasse dem gennem Navi-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Navi:
...
Du kan også tilpasse modeller gennem Navi-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med Navi. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `Navi.json`-filen i projektkataloget:
...
`limit`-felterne lader Navi forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `Navi auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek Navi-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i Navi-konfigurationen.
