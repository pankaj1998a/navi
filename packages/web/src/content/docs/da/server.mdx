---
title: Server
description: Interager med Navi-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `Navi serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en Navi-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `Navi.local` |
...
Indstil `Navi_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `Navi`, eller indstil `Navi_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `Navi serve` og `Navi web`.
...
Når du kører `Navi` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug Navi-serveren til at interagere med Navi programmatisk.
:::

Denne arkitektur lader Navi understøtte flere klienter og lader dig interagere med Navi programmatisk.

Du kan køre `Navi serve` for at starte en selvstændig server. Hvis du har
Navi TUI kørende, vil `Navi serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Navi [IDE](/docs/ide) plugins.
...
Navi-serveren viser følgende API'er.
