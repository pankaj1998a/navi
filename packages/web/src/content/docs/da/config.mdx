---
title: Konfiguration
description: Ved at bruge Navi JSON konfig.
---

Du kan konfigurere Navi ved hjælp af en JSON-konfigurationsfil.

---

## Format

Navi understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/Navi`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/Navi/Navi.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`Navi_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`Navi.json` i projekt) - projektspecifikke indstillinger
5. **`.Navi` mapper** - agent, kommandoer, plugins
6. **Inline config** (`Navi_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.Navi` og `~/.config/Navi` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/Navi`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale Navi-konfiguration i `~/.config/Navi/Navi.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `Navi.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når Navi starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `Navi_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `Navi_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.Navi` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.Navi` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`Navi.ai/config.json`**](https://Navi.ai/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `Navi serve` og `Navi web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din Navi-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `Navi.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din Navi-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger Navi at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din Navi-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/Navi/agents/` eller `.Navi/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil Navi falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`Navi run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/Navi/commands/` eller `.Navi/commands/`. [Learn more here](/docs/commands).
...
Navi vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard Navi **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide Navi med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.Navi/plugins/` eller `~/.config/Navi/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse Navi til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
