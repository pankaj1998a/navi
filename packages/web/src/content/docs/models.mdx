...
Most popular providers are preloaded by default. If you've added the credentials for a provider through the `/connect` command, they'll be available when you start Navi.
...
Here are several models that work well with Navi, in no particular order. (This is not an exhaustive list nor is it necessarily up to date):
...
Here the full ID is `provider_id/model_id`. For example, if you're using [Navi Zen](/docs/zen), you would use `navi/gpt-5.4-codex` if that model is exposed in your catalog.
...
Here we're configuring global settings for two built-in models: `gpt-5.4` when accessed via the `openai` provider, and `claude-sonnet-4-20250514` when accessed via the `anthropic` provider.
...
Many models support multiple variants with different configurations. Navi ships with built-in default variants for popular providers.

### Built-in variants

Navi ships with default variants for many providers:
...
When Navi starts up, it checks for models in the following priority order:

1. The `--model` or `-m` command line flag. The format is the same as in the config file: `provider_id/model_id`.

2. The model list in the Navi config.
