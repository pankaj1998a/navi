---
title: الخادم
description: تفاعل مع خادم Navi عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `Navi serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل Navi استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `Navi.local` |
...
عيّن `Navi_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `Navi`، أو عيّن `Navi_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `Navi serve` و `Navi web`.
...
عند تشغيل `Navi` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم Navi للتفاعل مع Navi برمجيا.
:::

تتيح هذه البنية لـ Navi دعم عدة عملاء وتمكّنك من التفاعل مع Navi برمجيا.

يمكنك تشغيل `Navi serve` لبدء خادم مستقل. إذا كانت واجهة Navi في terminal (TUI)
قيد التشغيل، فسيبدأ `Navi serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Navi لـ [IDE](/docs/ide).
...
يعرض خادم Navi واجهات API التالية.
