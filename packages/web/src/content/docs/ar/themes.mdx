...
مع Navi يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Navi سمتنا `Navi`.
...
- يريدون أن يطابق Navi مظهر terminal لديهم
...
يدعم Navi نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/Navi/themes/*.json` أو `$XDG_CONFIG_HOME/Navi/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.Navi/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.Navi/themes/*.json`
