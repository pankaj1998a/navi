---
title: المزوّدون
description: استخدام أي موفّر LLM في Navi.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Navi ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Navi.
...
في `~/.local/share/Navi/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Navi.
...
Navi Zen هي قائمة نماذج يوفّرها فريق Navi وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Navi. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Navi Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر Navi، ثم انتقل إلى [Navi.ai/auth](https://Navi.ai/auth).
...
يعمل مثل أي مزوّد آخر في Navi واستخدامه اختياري بالكامل.

---

## Navi Go

Navi Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Navi، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Navi.

1. شغّل الأمر `/connect` في TUI، واختر `Navi Go`، ثم انتقل إلى [Navi.ai/auth](https://Navi.ai/zen).
...
لاستخدام Amazon Bedrock مع Navi:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل Navi:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `Navi.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Navi غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين Navi لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل Navi بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات Navi.
...
تخزين الرمز في مساحة تخزين مصادقة Navi.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Navi نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Navi
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`Navi.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [Navi-gitlab-auth](https://www.npmjs.com/package/Navi-gitlab-auth).
...
لاستخدام اشتراك GitHub Copilot مع Navi:
...
لاستخدام Google Vertex AI مع Navi:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر Navi، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [Navi-helicone-session](https://github.com/H2Shami/Navi-helicone-session) لتسجيل كل محادثة Navi تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Navi مدرجة كجلسة مستقلة.
...
يمكنك ضبط Navi لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط Navi لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط Navi لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Navi. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/Navi) للتفاصيل.
:::
...
3. انسخ مفتاح API لاستخدامه في Navi.
...
6. **مهم**: قبل استخدام نماذج السحابة في Navi، يجب سحب معلومات النموذج محليا:
...
Navi Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Navi. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Navi Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Navi Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Navi.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات Navi.
...
5. يمكنك أيضا تخصيصها عبر إعدادات Navi. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Navi:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات Navi. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع Navi. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `Navi.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Navi بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `Navi auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات Navi وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات Navi.
