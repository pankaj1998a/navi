...
أنشئ الملف `.Navi/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Navi أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Navi:
...
- على مستوى النظام: `~/.config/Navi/commands/`
- لكل مشروع: `.Navi/commands/`
...
يتضمن Navi عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
