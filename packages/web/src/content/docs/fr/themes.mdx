...
Avec Navi, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Navi utilise notre propre thème `Navi`.
...
Navi est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Navi corresponde à l'apparence de leur terminal
...
Navi prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/Navi/themes/*.json` ou `$XDG_CONFIG_HOME/Navi/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.Navi/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.Navi/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/Navi/themes
vim ~/.config/Navi/themes/my-theme.json
```
...
{
  "$schema": "https://Navi.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
