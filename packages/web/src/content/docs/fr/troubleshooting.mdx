...
Pour déboguer les problèmes avec Navi, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\Navi\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `Navi --log-level DEBUG`.

---

## Stockage

Navi stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/Navi/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\Navi`
...
Navi Desktop exécute un serveur Navi local (le side-car `Navi-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `Navi` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/Navi/Navi.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\Navi\Navi.jsonc`
...
Navi peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/Navi/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\Navi\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Navi puisse le reconstruire.

1. Quittez complètement Navi Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/Navi`
- **Linux** : supprimez `~/.cache/Navi` (ou exécutez `rm -rf ~/.cache/Navi`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\Navi`

3. Redémarrez le bureau Navi.

---

### Résoudre les problèmes de connexion au serveur

Navi Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `Navi.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `Navi_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `Navi_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, Navi Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de Navi.

---

### Les notifications ne s'affichent pas

Navi Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Navi dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Navi.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Navi Desktop) :

- `Navi.settings.dat` (serveur de bureau par défaut URL)
- `Navi.global.dat` et `Navi.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec Navi :
...
3. Assurez-vous d'avoir la dernière version avec `Navi upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `Navi models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\Navi`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. Navi installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\Navi`

2. Redémarrez Navi pour réinstaller les derniers packages du fournisseur

Cela forcera Navi à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
Navi détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
