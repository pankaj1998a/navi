...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez Navi.
...
Voici plusieurs modèles qui fonctionnent bien avec Navi, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration Navi.

```json title="Navi.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [Navi Zen](/docs/zen), vous utiliserez `Navi/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="Navi.jsonc" {7-12,19-24}
{
  "$schema": "https://Navi.ai/config.json",
  "provider": {
    "openai": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. Navi est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

Navi est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
Lorsque Navi démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration Navi.
