---
title: Serveur
description: Interagissez avec le serveur Navi via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `Navi serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client Navi peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `Navi.local` |
...
Définissez `Navi_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `Navi`, ou définissez `Navi_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `Navi serve` et à `Navi web`.
...
Lorsque vous exécutez `Navi`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur Navi pour interagir avec Navi par programmation.
:::

Cette architecture permet à Navi de prendre en charge plusieurs clients et vous permet d'interagir avec Navi par programme.

Vous pouvez exécuter `Navi serve` pour démarrer un serveur autonome. Si vous avez le
Navi TUI en cours d'exécution, `Navi serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Navi [IDE](/docs/ide).
...
Le serveur Navi expose les API suivantes.
