---
title: Plugins
description: Écrivez vos propres plugins pour étendre Navi.
---

Les plugins vous permettent d'étendre Navi en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Navi.
...
- `.Navi/plugins/` - Plugins au niveau du projet
- `~/.config/Navi/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/Navi/node_modules/`.
...
1. Configuration globale (`~/.config/Navi/Navi.json`)
2. Configuration du projet (`Navi.json`)
3. Répertoire global des plugins (`~/.config/Navi/plugins/`)
4. Répertoire des plugins du projet (`.Navi/plugins/`)
...
Navi exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK Navi pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre Navi.
...
:::note
Si vous utilisez l'application de bureau Navi, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez Navi de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à Navi :
...
L'assistant `tool` crée un outil personnalisé que Navi peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour Navi aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://Navi.ai/docs/sdk) pour plus de détails.
