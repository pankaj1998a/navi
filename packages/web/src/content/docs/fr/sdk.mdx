---
title: SDK
description: Client JS de type sécurisé pour le serveur Navi.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK Navi JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler Navi par programme.
...
Créez une instance de Navi :

```javascript
import { createNavi } from "@Navi-ai/sdk"

const { client } = await createNavi()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `Navi.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${Navi.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de Navi, vous pouvez créer une instance client pour vous y connecter :
