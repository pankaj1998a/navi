...
Создайте `.Navi/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию Navi или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем Navi [config](/docs/config):
...
- Глобальный: `~/.config/Navi/commands/`
- Для каждого проекта: `.Navi/commands/`
...
Navi включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
