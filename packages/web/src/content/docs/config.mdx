---
title: Config
description: Using the Navi JSON config.
---

You can configure Navi using a JSON config file.

---

## Format

Navi supports both **JSON** and **JSONC** (JSON with Comments) formats.
...
1. **Remote config** (from `.well-known/Navi`) - organizational defaults
2. **Global config** (`~/.config/Navi/Navi.json`) - user preferences
3. **Custom config** (`Navi_CONFIG` env var) - custom overrides
4. **Project config** (`Navi.json` in project) - project-specific settings
5. **`.Navi` directories** - agents, commands, plugins
6. **Inline config** (`Navi_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/Navi/` on macOS) - admin-controlled
...
:::note
The `.Navi` and `~/.config/Navi` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/Navi` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
Place your global Navi config in `~/.config/Navi/Navi.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/Navi/tui.json`.
...
Add `Navi.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Navi starts up, it looks for a config file in the current directory or traverse up to the nearest Git directory.
...
Specify a custom config file path using the `Navi_CONFIG` environment variable.
...
Specify a custom config directory using the `Navi_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.Navi` directory, and should
...
The custom directory is loaded after the global config and `.Navi` directories, so it **can override** their settings.
...
Drop an `Navi.json` or `Navi.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/Navi/` |
| Linux    | `/etc/Navi/`                         |
| Windows  | `%ProgramData%\Navi`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Navi reads managed preferences from the `ai.Navi.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Navi checks these paths:

1. `/Library/Managed Preferences/<user>/ai.Navi.managed.plist`
2. `/Library/Managed Preferences/ai.Navi.managed.plist`

The plist keys map directly to `Navi.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.Navi.managed` PayloadType. The Navi config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.Navi.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.Navi.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.Navi</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`Navi.ai/config.json`**](https://Navi.ai/config.json).

TUI config uses [**`Navi.ai/tui.json`**](https://Navi.ai/tui.json).
...
Use `Navi_TUI_CONFIG` to point to a custom TUI config file.

Legacy `theme`, `keybinds`, and `tui` keys in `Navi.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `Navi serve` and `Navi web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Navi server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `Navi.local`. Useful for running multiple instances on the same network.
...
If not specified, Navi will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Navi config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Navi tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
You can also define agents using markdown files in `~/.config/Navi/agents/` or `.Navi/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Navi will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`Navi run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/Navi/commands/` or `.Navi/commands/`. [Learn more here](/docs/commands).
...
Navi uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Navi will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, Navi **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Navi with custom tools, hooks, and integrations.

Place plugin files in `.Navi/plugins/` or `~/.config/Navi/plugins/`. You can also load plugins from npm through the `plugin` option.
...
This is useful when you want to restrict Navi to only use specific providers rather than disabling them one by one.
