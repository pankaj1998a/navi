...
Puoi aggiungere strumenti esterni a Navi usando il _Model Context Protocol_, o MCP. Navi supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Navi Config](https://Navi.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/Navi`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Navi gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Navi:
...
Se il server richiede autenticazione, Navi ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `Navi mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Navi memorizzera' i token in modo sicuro in `~/.local/share/Navi/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Navi insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Navi come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Navi al tuo account Sentry.
