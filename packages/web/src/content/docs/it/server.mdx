---
title: Server
description: Interagisci con il server Navi via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `Navi serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client Navi.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `Navi.local` |
...
Imposta `Navi_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `Navi`, oppure imposta `Navi_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `Navi serve` sia per `Navi web`.
...
Quando esegui `Navi` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server Navi per interagire con Navi in modo programmatico.
:::

Questa architettura permette a Navi di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `Navi serve` per avviare un server standalone. Se la TUI di Navi e gia in esecuzione, `Navi serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Navi.
...
Il server Navi espone le seguenti API.
