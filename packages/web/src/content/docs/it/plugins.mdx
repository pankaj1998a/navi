---
title: Plugin
description: Scrivi plugin per estendere Navi.
---

I plugin ti permettono di estendere Navi agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Navi.
...
- `.Navi/plugins/` - plugin a livello progetto
- `~/.config/Navi/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/Navi/node_modules/`.
...
1. Config globale (`~/.config/Navi/Navi.json`)
2. Config di progetto (`Navi.json`)
3. Directory plugin globale (`~/.config/Navi/plugins/`)
4. Directory plugin di progetto (`.Navi/plugins/`)
...
Navi esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di Navi per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere Navi.
...
:::note
Se usi l'app desktop di Navi, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a Navi di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a Navi:
...
L'helper `tool` crea uno strumento personalizzato che Navi può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in Navi insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://Navi.ai/docs/sdk) per i dettagli.
