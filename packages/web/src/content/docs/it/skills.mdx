...
Le skill degli agenti permettono a Navi di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Navi cerca in queste posizioni:

- Config di progetto: `.Navi/skills/<name>/SKILL.md`
- Config globale: `~/.config/Navi/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Navi risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.Navi/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/Navi/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.Navi/skills/git-release/SKILL.md` cosi':
...
Navi elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `Navi.json`:
...
**Per agenti integrati** (in `Navi.json`):
