---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Navi.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Navi fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Navi avvia la TUI per la directory corrente.
...
Quando usi la TUI di Navi, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Navi. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Questo è separato da `Navi.json`, che configura il comportamento di server/runtime.
...
Usa `Navi_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
