---
title: TUI
description: Navi terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Navi, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

Navi'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
Navi TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
Navi'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
Navi'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `Navi.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `Navi_TUI_CONFIG` kullanın.
