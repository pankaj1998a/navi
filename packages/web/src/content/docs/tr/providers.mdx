---
title: Sağlayıcılar
description: Navi'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

Navi, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı Navi yapılandırmanızda yapılandırın.
...
`~/.local/share/Navi/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları Navi'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Navi Zen, Navi ekibi tarafından sağlanan modellerin bir listesidir.
Navi ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Navi Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, Navi'u seçin ve [Navi.ai/auth](https://Navi.ai/auth)'ye gidin.
...
Navi'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Navi Go

Navi Go, Navi ile iyi çalıştığı test edilmiş ve doğrulanmış, Navi ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Navi Go`'yu seçin ve [Navi.ai/auth](https://Navi.ai/zen) adresine gidin.
...
Amazon Bedrock'u Navi ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   Navi'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `Navi.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi Navi'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
Navi'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   Navi'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Navi yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in Navi auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
Navi, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. Navi'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`Navi.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [Navi-gitlab-auth](https://www.npmjs.com/package/Navi-gitlab-auth) ana sayfasında.

##### Yapılandırma

`Navi.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi Navi'la kullanmak için:
...
Google Vertex AI'yi Navi ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları Navi çalıştırılırken ayarlayın.
...
Helicone'un Navi aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her Navi görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [Navi-helicone-session](https://github.com/H2Shami/Navi-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her Navi görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
Navi'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla Navi'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla Navi'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini Navi için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/Navi)'a bakın.
:::
...
Ollama Cloud'u Navi ile kullanmak için:
...
3. Navi'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: Navi'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Navi Zen, Navi ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Navi Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Navi Zen**'i arayın.

   ```txt
   /connect
   ```

3. Navi API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   Navi yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca Navi yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Navi ile kullanmak için:
...
Modelleri Navi yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı Navi ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `Navi.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları Navi'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `Navi auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için Navi yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, Navi yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
