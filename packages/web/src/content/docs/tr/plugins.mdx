---
title: Eklentiler
description: Navi'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Navi'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Navi'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.Navi/plugins/` - Proje düzeyinde eklentiler
- `~/.config/Navi/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/Navi/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/Navi/Navi.json`)
2. Project config (`Navi.json`)
3. Global eklenti dizini (`~/.config/Navi/plugins/`)
4. Proje eklenti dizini (`.Navi/plugins/`)
...
Navi bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Navi SDK istemcisi.
...
Navi'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Navi masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Navi'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Navi'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Navi'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Navi'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://Navi.ai/docs/sdk) bakın.
