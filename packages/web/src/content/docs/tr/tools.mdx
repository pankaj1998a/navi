...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. Navi yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
Navi içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `Navi_EXPERIMENTAL_LSP_TOOL=true` (veya `Navi_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca Navi provider kullanılırken veya `Navi_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

Navi başlatırken etkinleştirmek için:
