...
Navi ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak Navi kendi `Navi` temasını kullanır.
...
Navi birden fazla yerleşik temayla gelir.
...
- Navi'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
Navi, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/Navi/themes/*.json` veya `$XDG_CONFIG_HOME/Navi/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.Navi/themes/*.json`
4. **Geçerli çalışma dizini** - `./.Navi/themes/*.json`
