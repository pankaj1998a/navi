...
_Model Bağlam Protokolü_ veya MCP kullanarak Navi'a harici araçlar ekleyebilirsiniz. Navi hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [Navi Config](https://Navi.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/Navi` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
Navi, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde Navi şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa Navi, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `Navi mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından Navi, belirteçleri `~/.local/share/Navi/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra Navi'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi Navi yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve Navi'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
