...
Navi Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
Navi Go şu anda beta olarak mevcut.
:::

Go, Navi'daki diğer sağlayıcılar gibi çalışır. Navi Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Navi'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Navi Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Navi Go, Navi'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Navi Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Navi Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Navi Go'ya abone olabilir.
:::
...
Navi Go aşağıdaki limitleri içerir:
...
Navi yapılandırmanızdaki [model id](/docs/config/#models) formatı `Navi-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `Navi-go/kimi-k2.6` kullanmalısınız.
...
Navi Go'yu şu amaçlarla oluşturduk:
...
4. Navi ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
