...
As ferramentas permitem que o LLM execute ações em sua base de código. O Navi vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no Navi.
...
:::note
Esta ferramenta está disponível apenas quando `Navi_EXPERIMENTAL_LSP_TOOL=true` (ou `Navi_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor Navi ou quando a variável de ambiente `Navi_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o Navi:
