---
title: Servidor
description: Interaja com o servidor Navi via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `Navi serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente Navi pode usar.
...
```bash
Navi serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `Navi.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
Navi serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `Navi_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `Navi`, ou defina `Navi_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `Navi serve` quanto ao `Navi web`.

```bash
Navi_SERVER_PASSWORD=your-password Navi serve
```
...
Quando você executa `Navi`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor Navi para interagir com o Navi programaticamente.
:::

Essa arquitetura permite que o Navi suporte múltiplos clientes e permite que você interaja com o Navi programaticamente.

Você pode executar `Navi serve` para iniciar um servidor autônomo. Se você tiver o TUI do Navi em execução, `Navi serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do Navi [IDE](/docs/ide).
...
O servidor Navi expõe as seguintes APIs.
