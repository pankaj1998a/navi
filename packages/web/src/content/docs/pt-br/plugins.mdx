---
title: Plugins
description: Escreva seus próprios plugins para estender o Navi.
---

Plugins permitem que você estenda o Navi conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do Navi.
...
- `.Navi/plugins/` - Plugins em nível de projeto
- `~/.config/Navi/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/Navi/node_modules/`.
...
1. Configuração global (`~/.config/Navi/Navi.json`)
2. Configuração do projeto (`Navi.json`)
3. Diretório de plugins global (`~/.config/Navi/plugins/`)
4. Diretório de plugins do projeto (`.Navi/plugins/`)
...
O Navi executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do Navi para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o Navi.
...
:::note
Se você estiver usando o aplicativo desktop Navi, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o Navi de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao Navi:
...
O helper `tool` cria uma ferramenta personalizada que o Navi pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o Navi junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://Navi.ai/docs/sdk) para detalhes.
