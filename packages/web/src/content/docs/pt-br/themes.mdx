...
Com o Navi, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o Navi usa nosso próprio tema `Navi`.
...
O Navi vem com vários temas embutidos.
...
- Querem que o Navi corresponda à aparência do seu terminal
...
O Navi suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/Navi/themes/*.json` ou `$XDG_CONFIG_HOME/Navi/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.Navi/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.Navi/themes/*.json`
