...
As habilidades do agente permitem que o Navi descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O Navi pesquisa nesses locais:

- Configuração do projeto: `.Navi/skills/<name>/SKILL.md`
- Configuração global: `~/.config/Navi/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o Navi sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.Navi/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/Navi/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.Navi/skills/git-release/SKILL.md` assim:
...
O Navi lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `Navi.json`:
...
**Para agentes embutidos** (em `Navi.json`):
