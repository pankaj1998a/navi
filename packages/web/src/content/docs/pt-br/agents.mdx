...
Existem dois tipos de agentes no Navi; agentes primários e subagentes.
...
Navi vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Navi vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Navi vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Navi, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `Navi.json`:
...
- Global: `~/.config/Navi/agents/`
- Por projeto: `.Navi/agents/`

```markdown title="~/.config/Navi/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o Navi usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do Navi quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do Navi usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Navi Zen](/docs/zen), você usaria `Navi/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `Navi models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/Navi).
:::
...
```markdown title="~/.config/Navi/agents/docs-writer.md"
---
...
```markdown title="~/.config/Navi/agents/security-auditor.md"
---
