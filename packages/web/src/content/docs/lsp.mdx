---
title: LSP Servers
description: Navi integrates with your LSP servers.
---

Navi can integrate with your Language Server Protocol (LSP) to help the LLM interact with your codebase. It uses diagnostics to provide feedback to the LLM.

---

## Built-in

Navi comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `Navi_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and Navi opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your Navi config.
