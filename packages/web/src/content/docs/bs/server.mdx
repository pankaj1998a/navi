---
title: Server
description: Komunicirajte s Navi serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `Navi serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji Navi klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `Navi.local` |
...
Postavite `Navi_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `Navi`, ili postavite `Navi_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `Navi serve` i za `Navi web`.
...
Kada pokrenete `Navi`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite Navi server za programsku interakciju sa Navi-om.
:::

Ova arhitektura omogucava Navi podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `Navi serve` da startate standalone server. Ako je Navi TUI vec pokrenut, `Navi serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Navi [IDE](/docs/ide) pluginovi.
...
Navi server izlaže sljedece API-je.
