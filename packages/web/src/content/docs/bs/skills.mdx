...
Agent skills omogucavaju Navi da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Navi pretrazuje ove lokacije:

- Konfiguracija projekta: `.Navi/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/Navi/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Navi ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.Navi/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/Navi/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.Navi/skills/git-release/SKILL.md` ovako:
...
Navi navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `Navi.json`:
...
**Za ugradene agente** (u `Navi.json`):
