...
Da biste otklonili probleme s Navi, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\Navi\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `Navi --log-level DEBUG`.

---

## Pohrana

Navi pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/Navi/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\Navi`
...
Navi Desktop pokreće lokalni Navi server (`Navi-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Navi` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/Navi/Navi.jsonc` (ili `~/.config/Navi/Navi.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/Navi/Navi.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\Navi\Navi.jsonc`
...
Navi također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/Navi/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\Navi\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Navi mogao ponovo izgraditi.

1. Potpuno zatvorite Navi Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/Navi`
- **Linux**: obrišite `~/.cache/Navi` (ili pokrenite `rm -rf ~/.cache/Navi`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\Navi`

3. Ponovo pokrenite Navi Desktop.

---

### Rješavanje problema sa vezom na serveru

Navi Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `Navi.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `Navi_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `Navi_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Navi Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Navi karakteristikama.

---

### Obavještenja se ne prikazuju

Navi Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Navi u postavkama vašeg OS-a, i
...
1. Zatvorite Navi Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Navi Desktop aplikacije):

- `Navi.settings.dat` (URL zadanog servera za desktop)
- `Navi.global.dat` i `Navi.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `Navi upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `Navi models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\Navi`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Navi dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\Navi`

2. Ponovo pokrenite Navi da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Navi da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Navi će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
