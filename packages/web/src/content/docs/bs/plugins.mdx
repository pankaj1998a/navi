---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Navi.
---

Dodaci vam omogućavaju da proširite Navi spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Navi.
...
- `.Navi/plugins/` - Dodaci na nivou projekta
- `~/.config/Navi/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/Navi/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/Navi/Navi.json`)
2. Konfiguracija projekta (`Navi.json`)
3. Globalni direktorij dodataka (`~/.config/Navi/plugins/`)
4. Direktorij dodataka projekta (`.Navi/plugins/`)
...
Navi pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Navi SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Navi.
...
Spriječite Navi da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u Navi:
...
Pomoćnik `tool` kreira prilagođeni alat koji Navi može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za Navi zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://Navi.ai/docs/sdk) za detalje.
