---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Navi.
---

import config from "../../../../config.mjs"
export const console = config.console

Navi koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Navi konfiguraciji.
...
u `~/.local/share/Navi/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Navi
config.
...
Navi Zen je lista modela koje je obezbedio Navi tim koji su bili
testirano i potvrđeno da dobro radi sa Navi. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Navi Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite Navi i idite na [Navi.ai/auth](https://Navi.ai/auth).
...
Radi kao i svaki drugi provajder u Navi i potpuno je opcionalan za korištenje.

---

## Navi Go

Navi Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Navi tim i koji su testirani i verificirani da dobro rade s Navi-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Navi Go` i idite na [Navi.ai/auth](https://Navi.ai/zen).
...
Da biste koristili Amazon Bedrock s Navi:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete Navi:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `Navi.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Navi.
:::
...
Možete konfigurirati Navi za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi Navi ispravno radio.
   :::
...
Također možete dodati modele kroz svoju Navi konfiguraciju.
...
da pohrani token u Navi auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Navi koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Navi
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`Navi.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [Navi-gitlab-auth](https://www.npmjs.com/package/Navi-gitlab-auth).

##### Konfiguracija

Prilagodite putem `Navi.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s Navi:
...
Za korištenje Google Vertex AI s Navi:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće Navi.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem Navia, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [Navi-helicone-session](https://github.com/H2Shami/Navi-helicone-session) da automatski prijavite svaki Navi razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Navi razgovor naveden kao zasebna sesija.
...
Možete konfigurirati Navi za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati Navi za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati Navi za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Navi. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/Navi) za detalje.
:::
...
Da biste koristili Ollama Cloud s Navi:
...
3. Kopirajte API ključ za korištenje u Navi.
...
6. **Važno**: Prije upotrebe modela oblaka u Navi, morate lokalno povući informacije o modelu:
...
Navi Zen je lista testiranih i verifikovanih modela koju je obezbedio Navi tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Navi Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Navi Zen**.

```txt
   /connect
```

3. Unesite svoj Navi API ključ.
...
Također možete dodati dodatne modele putem vaše Navi konfiguracije.
...
5. Također ih možete prilagoditi putem vaše Navi konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Naviom:
...
Također možete prilagoditi modele kroz svoju Navi konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s Navi-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `Navi.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Navi da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `Navi auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Navi konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj Navi konfiguraciji.
