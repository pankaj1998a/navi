---
title: Konfiguracija
description: Korištenje Navi JSON konfiguracije.
---

Možete konfigurirati Navi koristeći JSON konfiguracijski fajl.

---

## Format

Navi podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/Navi`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/Navi/Navi.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`Navi_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`Navi.json` u projektu) - postavke specifične za projekat
5. **`.Navi` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`Navi_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.Navi` i `~/.config/Navi` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/Navi` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Navi konfiguraciju u `~/.config/Navi/Navi.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/Navi/tui.json`.
...
Dodajte `Navi.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Navi pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `Navi_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `Navi_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.Navi` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.Navi`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`Navi.ai/config.json`**](https://Navi.ai/config.json).

TUI konfiguracija koristi [**`Navi.ai/tui.json`**](https://Navi.ai/tui.json).
...
Koristite `Navi_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `Navi.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `Navi serve` i `Navi web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Navi server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `Navi.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Navi konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Navi pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/Navi/agents/` ili `.Navi/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Navi će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`Navi run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/Navi/commands/` ili `.Navi/commands/`. [Saznajte više ovdje](/docs/commands).
...
Navi će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Navi **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Navi sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.Navi/plugins/` ili `~/.config/Navi/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Navi da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
