...
Možete dodati vanjske alate u Navi koristeći _Model Context Protocol_, ili MCP. Navi podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Navi Config](https://Navi.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/Navi`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Navi automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Navi će:
...
Ako server zahtijeva autentifikaciju, Navi će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `Navi mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Navi sigurno čuva tokene u `~/.local/share/Navi/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Navi, zajedno s ugrađenim alatima. Možete njima upravljati kroz Navi konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Navi sa vašim Sentry nalogom.
