...
U Navi mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Navi koristi nasu `Navi` temu.
...
Navi dolazi sa vise ugradenih tema.
...
- Zele da Navi odgovara izgledu njihovog terminala
...
Navi podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/Navi/themes/*.json` ili `$XDG_CONFIG_HOME/Navi/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.Navi/themes/*.json`
4. **Trenutni radni direktorij** - `./.Navi/themes/*.json`
