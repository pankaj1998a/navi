...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Navi dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Navi.
...
:::note
Ovaj alat je dostupan samo kada je `Navi_EXPERIMENTAL_LSP_TOOL=true` (ili `Navi_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Navi provajdera ili kada je varijabla `Navi_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Navi:
