...
Aby debugować problemy z Navi, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\Navi\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `Navi --log-level DEBUG`.

---

## Przechowywanie danych

Navi przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/Navi/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\Navi`
...
Navi Desktop uruchamia w tle lokalny serwer Navi (sidecar `Navi-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Navi` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/Navi/Navi.jsonc` (lub `~/.config/Navi/Navi.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/Navi/Navi.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\Navi\Navi.jsonc`
...
Navi może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/Navi/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\Navi\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby Navi mógł ją odbudować.

1. Całkowicie zamknij Navi Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/Navi`
- **Linux**: usuń `~/.cache/Navi` (lub uruchom `rm -rf ~/.cache/Navi`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\Navi`

3. Uruchom ponownie Navi Desktop.

---

### Napraw problemy z połączeniem z serwerem

Navi Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `Navi.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `Navi_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `Navi_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows Navi Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami Navi.

---

### Brak powiadomień

Navi Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla Navi w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie Navi Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji Navi Desktop):

- `Navi.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `Navi.global.dat` i `Navi.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `Navi upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `Navi models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\Navi`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. Navi dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\Navi`

2. Uruchom ponownie kod Navi, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to Navi do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
Navi wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
