...
Dzięki Navi możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie Navi używa naszego własnego motywu `Navi`.
...
Navi ma kilka wbudowanych motywów.
...
- Chcą, aby Navi pasował do wyglądu ich terminala
...
Navi obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/Navi/themes/*.json` lub `$XDG_CONFIG_HOME/Navi/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.Navi/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.Navi/themes/*.json`
