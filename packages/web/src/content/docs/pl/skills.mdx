...
Umiejętności agenta pozwalają Navi odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
Navi przeszukuje te lokalizacje:

- Project config: `.Navi/skills/<name>/SKILL.md`
- Global config: `~/.config/Navi/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu Navi przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.Navi/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/Navi/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.Navi/skills/git-release/SKILL.md` w ten sposób:
...
Navi wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `Navi.json`:
...
**Dla agentów wbudowanych** (w `Navi.json`):
