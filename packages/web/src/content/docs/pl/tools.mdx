...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Navi posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Navi.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `Navi_EXPERIMENTAL_LSP_TOOL=true` (lub `Navi_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Navi lub gdy zmienna środowiskowa `Navi_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Navi:
