---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć Navi.
---

Wtyczki umożliwiające wyłączenie Navi poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania Navi.
...
- `.Navi/plugins/` - Wtyczki na poziomie projektu
- `~/.config/Navi/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/Navi/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/Navi/Navi.json`)
2. Konfiguracja projektu (`Navi.json`)
3. Globalny katalog wtyczek (`~/.config/Navi/plugins/`)
4. Katalog wtyczek projektu (`.Navi/plugins/`)
...
Navi uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z Navi do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do Navi.
...
:::note
Jeśli korzystasz z aplikacji komputerowej Navi, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj Navi czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do Navi:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod Navi. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://Navi.ai/docs/sdk).
