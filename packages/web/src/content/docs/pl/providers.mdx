---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w Navi.
---

import config from "../../../../config.mjs"
export const console = config.console

Navi używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji Navi.
...
w `~/.local/share/Navi/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim Navi
config.
...
Navi Zen to lista modeli dostarczonych przez zespół Navi, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z Navi. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Navi Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Navi Zen` i przejdź do [Navi.ai/auth](https://Navi.ai/zen).
...
Działa jak każdy inny dostawca w Navi i jest całkowicie opcjonalny w użyciu.

---

## Navi Go

Navi Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół Navi, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z Navi.

1. Uruchom polecenie `/connect` w TUI, wybierz `Navi Go` i przejdź do [Navi.ai/auth](https://Navi.ai/zen).
...
Aby używać Amazon Bedrock z Navi:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania Navi:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `Navi.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w Navi nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować Navi do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod Navi działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji Navi.
...
Navi używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować Navi
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`Navi.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [Navi-gitlab-auth](https://www.npmjs.com/package/Navi-gitlab-auth).

##### Konfiguracja

Customize through `Navi.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem Navi:
...
Aby używać Google Vertex AI z Navi:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania Navi.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą Navi, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [Navi-helicone-session](https://github.com/H2Shami/Navi-helicone-session), aby automatycznie rejestrować każdą konwersację Navi jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja Navi będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować Navi tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować Navi tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować Navi tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem Navi. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/Navi).
:::
...
Aby korzystać z Ollama Cloud z Navi:
...
3. Skopiuj klucz API do użycia w Navi.
...
6. **Ważne**: Przed użyciem modeli chmurowych w Navi musisz pobrać informacje o modelu lokalnie:
...
Navi Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół Navi. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Navi Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Navi Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API Navi.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji Navi.
...
5. Możesz je także dostosować za pomocą konfiguracji Navi. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z Navi:
...
Możesz także dostosować modele za pomocą konfiguracji Navi. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z Navi. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `Navi.json` w katalogu projektu:
...
Pola `limit` pozwalają Navi zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `Navi auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację Navi i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji Navi.
