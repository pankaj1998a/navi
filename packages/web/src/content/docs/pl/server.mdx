---
title: Serwer
description: Interakcja z serwerem Navi za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `Navi serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient Navi.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `Navi.local` |
...
Ustaw `Navi_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `Navi` lub ustaw `Navi_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `Navi serve`, jak i `Navi web`.
...
Kiedy uruchomisz `Navi`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera Navi do programowej interakcji z Navi.
:::

Ta architektura umożliwia obsługę wielu klientów przez Navi i programową interakcję z kodem otwartym.

Możesz uruchomić `Navi serve`, aby uruchomić samodzielny serwer. Jeśli masz
Navi TUI działa, `Navi serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki Navi [IDE](/docs/ide).
...
Serwer Navi udostępnia następujące interfejsy API.
