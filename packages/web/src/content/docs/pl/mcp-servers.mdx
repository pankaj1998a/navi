...
Możesz dodać dodatkowe informacje do Navi za pomocą _Model Context Protocol_, w skrócie MCP. Navi obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [Navi Config](https://Navi.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/Navi`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
Navi automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, Navi:
...
Jeśli serwer wymaga uwierzytelnienia, Navi poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `Navi mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji Navi bezpieczne przechowa tokeny w `~/.local/share/Navi/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w Navi, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie Navi, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć Navi z kontem Sentry.
