---
title: TUI
description: Using the Navi terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Navi provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Navi starts the TUI for the current directory.
...
When using the Navi TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Navi. Allows you to select from available providers and add their API keys.
...
This is separate from `Navi.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Navi waits after the leader key. Defaults to `2000`.
...
Use `Navi_TUI_CONFIG` to load a custom TUI config path.
