...
:::caution
Modes are now configured through the `agent` option in the navi config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in navi allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the navi config.
...
navi comes with two built-in modes.
...
Configure modes in your `navi.json` config file:
...
- Global: `~/.config/navi/mode/`
- Project: `.navi/mode/`

```markdown title="~/.config/navi/mode/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
both the global navi config and the project specific config.
...
Create mode files in `.navi/mode/` for project-specific modes or `~/.config/navi/mode/` for global modes:

```markdown title=".navi/mode/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/navi/mode/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
