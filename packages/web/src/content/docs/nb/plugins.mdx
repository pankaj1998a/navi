...
description: Skriv dine egne programtillegg for å utvide Navi.
---

Plugins lar deg utvide Navi ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Navi.
...
- `.Navi/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/Navi/node_modules/`.
...
1. Global konfigurasjon (`~/.config/Navi/Navi.json`)
2. Prosjektkonfigurasjon (`Navi.json`)
3. Global plugin-katalog (`~/.config/Navi/plugins/`)
4. Prosjektpluginkatalog (`.Navi/plugins/`)
...
Navi kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Navi SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Navi.
...
:::note
Hvis du bruker Navi-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Navi fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Navi:
...
`tool`-hjelperen lager et tilpasset verktøy som Navi kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Navi sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://Navi.ai/docs/sdk) for detaljer.
