---
title: Tjener
description: Samhandle med Navi-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `Navi serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Navi-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `Navi.local` |
...
Sett `Navi_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `Navi` som standard, eller sett `Navi_SERVER_USERNAME` for å overstyre det. Dette gjelder både `Navi serve` og `Navi web`.
...
Når du kjører `Navi` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Navi-serveren til å samhandle med Navi programmatisk.
:::

Denne arkitekturen lar Navi støtte flere klienter og lar deg samhandle med Navi programmatisk.

Du kan kjøre `Navi serve` for å starte en frittstående server. Hvis du har
Navi TUI kjører, vil `Navi serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Navi [IDE](/docs/ide) plugins.
...
Navi-serveren viser følgende APIer.
