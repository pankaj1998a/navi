...
Med Navi kan du velge fra ett av flere innebygde temaer, bruke et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruker Navi vårt eget `Navi`-tema.
...
Navi kommer med flere innebygde temaer.
...
- Vil at Navi skal matche terminalens utseende
...
Navi støtter et fleksibelt JSON-basert temasystem som lar brukere enkelt lage og tilpasse temaer.
...
2. **Brukerkonfigurasjonskatalog** - Definert i `~/.config/Navi/themes/*.json` eller `$XDG_CONFIG_HOME/Navi/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.Navi/themes/*.json`
4. **Gjeldende arbeidskatalog** - Definert i `./.Navi/themes/*.json`
