---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Navi.
---

import config from "../../../../config.mjs"
export const console = config.console

Navi bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Navi-konfigurasjonen.
...
i `~/.local/share/Navi/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Navi
konfig.
...
Navi Zen er en liste over modeller levert av Navi-teamet som har vært
testet og verifisert for å fungere godt med Navi. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Navi Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Navi Zen` og gå til [Navi.ai/auth](https://Navi.ai/zen).
...
Det fungerer som alle andre leverandører i Navi og er helt valgfritt å bruke.

---

## Navi Go

Navi Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Navi-teamet som har vært
testet og verifisert for å fungere godt med Navi.

1. Kjør kommandoen `/connect` i TUI, velg `Navi Go`, og gå til [Navi.ai/auth](https://Navi.ai/zen).
...
Slik bruker du Amazon Bedrock med Navi:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Navi:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `Navi.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i Navi støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere Navi til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Navi skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Navi-konfigurasjonen.
...
for å lagre token i Navi auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Navi bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Navi
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`Navi.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [Navi-gitlab-auth](https://www.npmjs.com/package/Navi-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `Navi.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Navi:
...
Slik bruker du Google Vertex AI med Navi:
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Navi, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [Navi-helicone-session](https://github.com/H2Shami/Navi-helicone-session) plugin for automatisk å logge hver Navi-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Navi-samtale oppført som en separat økt.
...
Du kan konfigurere Navi for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Navi for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Navi for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Navi. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/Navi) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Navi:
...
3. Kopier API-nøkkelen for bruk i Navi.
...
6. **Viktig**: Før du bruker skymodeller i Navi, må du hente modellinformasjonen lokalt:
...
Navi Zen er en liste over testede og verifiserte modeller levert av Navi-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Navi Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Navi Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Navi API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Navi-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Navi-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Navi:
...
Du kan også tilpasse modeller gjennom Navi-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Navi. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `Navi.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Navi forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `Navi auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Navi-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Navi-konfigurasjonen.
