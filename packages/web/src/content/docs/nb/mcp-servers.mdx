...
Du kan legge til eksterne verktøy i Navi ved å bruke _Model Context Protocol_ eller MCP. Navi støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Navi Config](https://Navi.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="Navi.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/Navi`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Navi håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Navi:
...
Hvis serveren krever autentisering, vil Navi be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `Navi mcp auth <server-name>`.
...
```bash
Navi mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Navi lagre tokens sikkert i `~/.local/share/Navi/mcp-auth.json`.
...
Navi mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Navi, sammen med innebygde verktøy. Så du kan administrere dem gjennom Navi-konfigurasjonen som alle andre verktøy.
...
```bash
Navi mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Navi til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Navi-samtale oppført som en separat økt.
