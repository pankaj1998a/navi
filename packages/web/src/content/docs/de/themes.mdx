...
Mit Navi kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Navi das `Navi`-Theme.
...
Navi liefert mehrere eingebaute Themes mit.
...
- Navi optisch ans Terminal anpassen wollen
...
Navi unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/Navi/themes/*.json` oder `$XDG_CONFIG_HOME/Navi/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.Navi/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.Navi/themes/*.json`
