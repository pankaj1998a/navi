---
title: Regeln
description: Lege eigene Anweisungen fuer Navi fest.
---

Du kannst Navi ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in Navi den Befehl `/init` aus.
...
So kann Navi sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht Navi sie zu erweitern.
...
Navi kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/Navi/AGENTS.md` speichern.
Diese gelten in allen Navi-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Navi dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/Navi/AGENTS.md` existiert)
...
Beim Start sucht Navi in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/Navi/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `Navi.json` oder in `~/.config/Navi/Navi.json` angeben.
...
Navi parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit Navi.json

Empfohlen ist das `instructions`-Feld in der `Navi.json`:
...
Du kannst Navi explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `Navi.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
