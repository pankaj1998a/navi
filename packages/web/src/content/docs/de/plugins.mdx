---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Navi zu erweitern.
---

Mit Plugins können Sie Navi erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Navi zu ändern.
...
- `.Navi/plugins/` – Plugins auf Projektebene
- `~/.config/Navi/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/Navi/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/Navi/Navi.json`)
2. Projektkonfiguration (`Navi.json`)
3. Globales Plugin-Verzeichnis (`~/.config/Navi/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.Navi/plugins/`)
...
Navi führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Navi-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Navi erweitern können.
...
:::note
Wenn Sie die Navi-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Navi `.env`-Dateien liest:
...
Plugins können Navi auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Navi aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Navi zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://Navi.ai/docs/sdk).
