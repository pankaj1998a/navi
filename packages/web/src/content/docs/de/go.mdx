...
Navi Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

:::note
Navi Go befindet sich derzeit in der Beta-Phase.
:::

Go funktioniert wie jeder andere Provider in Navi. Du abonnierst Navi Go und
...
Navi Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

Navi Go funktioniert wie jeder andere Provider in Navi.

1. Du meldest dich bei **<a href={console}>Navi Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Navi Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Navi Go abonnieren.
:::
...
Navi Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Navi Config verwendet das Format `Navi-go/<model-id>`. Für Kimi K2.6 würdest du beispielsweise `Navi-go/kimi-k2.6` in deiner Config verwenden.
...
Wir haben Navi Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Navi zu nutzen.
