...
Agent Skills erlauben Navi, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Navi sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Navi vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.Navi/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/Navi/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.Navi/skills/git-release/SKILL.md` zum Beispiel so:
...
Navi listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `Navi.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `Navi.json`):
