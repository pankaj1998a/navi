---
title: Server
description: Nutze den Navi-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `Navi serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein Navi-Client nutzen kann.
...
Setze `Navi_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `Navi`, kann aber mit `Navi_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `Navi serve` und `Navi web`.
...
Wenn du `Navi` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den Navi-Server, um Navi programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `Navi serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `Navi serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Navi-[IDE](/docs/ide)-Plugins.
...
Der Navi-Server stellt folgende APIs bereit.
