...
Sie können externe Tools zu Navi hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Navi unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Navi Config](https://Navi.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/Navi`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Navi übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Navi Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Navi beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `Navi mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Navi die Token sicher in `~/.local/share/Navi/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Navi verfügbar. Sie können sie also wie jedes andere Tool über die Navi-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Navi mit Ihrem Sentry-Konto zu verbinden.
