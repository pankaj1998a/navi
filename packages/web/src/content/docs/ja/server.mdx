---
title: サーバー
description: HTTP 経由で Navi サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`Navi serve` コマンドは、Navi クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `Navi.local` |
...
HTTP 基本認証でサーバーを保護するには、`Navi_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `Navi` になるか、`Navi_SERVER_USERNAME` を設定してオーバーライドします。これは、`Navi serve` と `Navi web` の両方に当てはまります。
...
`Navi` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Navi サーバーを使用して、プログラムで Navi と対話します。
:::

`Navi serve` を実行してスタンドアロンサーバーを起動できます。すでに Navi TUI を実行している場合でも、`Navi serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Navi [IDE](/docs/ide) プラグインによって使用されます。
