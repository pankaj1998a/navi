...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `navi`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `navi serve` and `navi web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password navi serve
```
