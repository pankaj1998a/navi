---
title: Server
description: Interact with Navi server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `Navi serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an Navi client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `Navi.local` |
...
Set `Navi_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `Navi`, or set `Navi_SERVER_USERNAME` to override it. This applies to both `Navi serve` and `Navi web`.
...
When you run `Navi` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the Navi server to interact with Navi programmatically.
:::

This architecture lets Navi support multiple clients and allows you to interact with Navi programmatically.

You can run `Navi serve` to start a standalone server. If you have the
Navi TUI running, `Navi serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Navi [IDE](/docs/ide) plugins.
...
The Navi server exposes the following APIs.
