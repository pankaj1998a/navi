...
Navi Go es una suscripción de bajo costo — **$5 por tu primer mes**, luego **$10/mes** — que te brinda acceso confiable a modelos abiertos de programación populares.

:::note
Navi Go está actualmente en beta.
:::

Go funciona como cualquier otro proveedor en Navi. Te suscribes a Navi Go y
...
Navi Go te da acceso a estos modelos por **$5 por tu primer mes**, luego **$10/mes**.

---

## Cómo funciona

Navi Go funciona como cualquier otro proveedor en Navi.

1. Inicias sesión en **<a href={console}>Navi Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `Navi Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a Navi Go.
:::
...
Navi Go incluye los siguientes límites:
...
El [ID del modelo](/docs/config/#models) en tu configuración de Navi
usa el formato `Navi-go/<model-id>`. Por ejemplo, para Kimi K2.6, usarías
`Navi-go/kimi-k2.6` en tu configuración.
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con Navi.
