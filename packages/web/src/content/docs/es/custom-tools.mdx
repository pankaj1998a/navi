...
description: Cree herramientas que LLM pueda llamar en Navi.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de Navi como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.Navi/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/Navi/tools/`.
...
```ts title=".Navi/tools/database.ts" {1}
...
```ts title=".Navi/tools/project.ts" {8}
import { tool } from "@Navi-ai/plugin"
...
```python title=".Navi/tools/add.py"
import sys
...
```ts title=".Navi/tools/python-add.ts" {10}
import { tool } from "@Navi-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".Navi/tools/add.py")
