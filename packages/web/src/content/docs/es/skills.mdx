...
Las habilidades del agente permiten a Navi descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Navi busca estas ubicaciones:

- Configuración del proyecto: `.Navi/skills/<name>/SKILL.md`
- Configuración global: `~/.config/Navi/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Navi sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.Navi/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/Navi/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.Navi/skills/git-release/SKILL.md` así:
...
Navi enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `Navi.json`:
...
**Para agentes integrados** (en `Navi.json`):
