...
Con Navi puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Navi usa nuestro propio tema `Navi`.
...
Navi viene con varios temas integrados.
...
- Quiere que Navi coincida con la apariencia de su terminal
...
Navi admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/Navi/themes/*.json` o `$XDG_CONFIG_HOME/Navi/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.Navi/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.Navi/themes/*.json`
