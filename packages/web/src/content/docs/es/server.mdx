---
title: Servidor
description: Interactuar con el servidor Navi a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `Navi serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente Navi puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `Navi.local` |
...
Configure `Navi_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `Navi`, o configure `Navi_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `Navi serve` como a `Navi web`.
...
Cuando ejecuta `Navi`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor Navi para interactuar con Navi mediante programación.
:::

Esta arquitectura permite que Navi admita múltiples clientes y le permite interactuar con Navi mediante programación.

Puede ejecutar `Navi serve` para iniciar un servidor independiente. Si tienes el
Navi TUI ejecutándose, `Navi serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Navi [IDE](/docs/ide).
...
El servidor Navi expone las siguientes API.
