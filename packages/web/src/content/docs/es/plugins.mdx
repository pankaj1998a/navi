---
title: Complementos
description: Escriba sus propios complementos para extender Navi.
---

Los complementos le permiten extender Navi al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Navi.
...
- `.Navi/plugins/` - Complementos a nivel de proyecto
- `~/.config/Navi/plugins/` - Complementos globales
...
```json title="Navi.json"
{
  ...
  "plugin": ["Navi-helicone-session", "Navi-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/Navi/node_modules/`.
...
1. Configuración global (`~/.config/Navi/Navi.json`)
2. Configuración del proyecto (`Navi.json`)
3. Directorio global de complementos (`~/.config/Navi/plugins/`)
4. Directorio de complementos del proyecto (`.Navi/plugins/`)
...
Navi ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".Navi/plugins/my-plugin.ts"
...
```js title=".Navi/plugins/example.js"
...
- `client`: Un cliente SDK Navi para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar Navi.
...
```js title=".Navi/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "Navi"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Navi, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que Navi lea archivos `.env`:

```javascript title=".Navi/plugins/env-protection.js"
...
```javascript title=".Navi/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a Navi:

```ts title=".Navi/plugins/custom-tools.ts"
import { type Plugin, tool } from "@Navi-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que Navi puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para Navi junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://Navi.ai/docs/sdk) para obtener más detalles.
...
```ts title=".Navi/plugins/compaction.ts"
import type { Plugin } from "@Navi-ai/plugin"
...
```ts title=".Navi/plugins/custom-compaction.ts"
import type { Plugin } from "@Navi-ai/plugin"
