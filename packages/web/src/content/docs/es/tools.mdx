...
Las herramientas permiten que LLM realice acciones en su código base. Navi viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Navi.
...
:::note
Esta herramienta solo está disponible cuando `Navi_EXPERIMENTAL_LSP_TOOL=true` (o `Navi_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Navi o cuando la variable de entorno `Navi_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Navi:

```bash
Navi_ENABLE_EXA=1 Navi
```
