...
- Locally by placing them in the `.navi/tool/` directory of your project.
- Or globally, by placing them in `~/.config/navi/tool/`.
...
```ts title=".navi/tool/database.ts" {1}
...
```ts title=".navi/tool/project.ts" {8}
import { tool } from "@navi-ai/plugin"
...
```ts title=".navi/tool/python-add.ts" {10}
import { tool } from "@navi-ai/plugin"
...
export default tool({
  ...
  async execute(args) {
    const result = await Bun.$`python3 .navi/tool/add.py ${args.a} ${args.b}`.text()
    return result.trim()
  },
})
```
