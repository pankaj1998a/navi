...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("navi-find")
  api.delete("navi-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("navi-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("navi-find", new Highlight(...rest))
    return true
  }
