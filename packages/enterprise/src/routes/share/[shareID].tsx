...
import { WorkerPoolProvider } from "@navi-ai/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createEffect, createMemo, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@navi-ai/ui/logo"
import { IconButton } from "@navi-ai/ui/icon-button"
import { ProviderIcon } from "@navi-ai/ui/provider-icon"
import { createDefaultOptions } from "@navi-ai/ui/pierre"
import { iife } from "@navi-ai/util/iife"
import { Binary } from "@navi-ai/util/binary"
import { NamedError } from "@navi-ai/util/error"
import { DateTime } from "luxon"
import { SessionMessageRail } from "@navi-ai/ui/session-message-rail"
import { createStore } from "solid-js/store"
import z from "zod"
import NotFound from "../[...404]"
import { Tabs } from "@navi-ai/ui/tabs"
import { preloadMultiFileDiff, PreloadMultiFileDiffResult } from "@pierre/diffs/ssr"
import { Diff as SSRDiff } from "@navi-ai/ui/diff-ssr"
import { clientOnly } from "@solidjs/start"
import { type IconName } from "@navi-ai/ui/icons/provider"
import { Meta, Title } from "@solidjs/meta"
import { Base64 } from "js-base64"

const ClientOnlyDiff = clientOnly(() => import("@navi-ai/ui/diff").then((m) => ({ default: m.Diff })))
const ClientOnlyCode = clientOnly(() => import("@navi-ai/ui/code").then((m) => ({ default: m.Code })))
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@navi-ai/ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/navi-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | Navi</Title>
              </Show>
              <Meta name="description" content="navi - The AI coding agent built for the terminal." />
                          ...
                          <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                            <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                              ...
                              <div class="flex gap-3 items-center">
                                <IconButton
                                  as={"a"}
                                  href="https://github.com/anomalyco/navi"
                                  target="_blank"
                                  icon="github"
                                  variant="ghost"
                                />
