...
- Author package entrypoint is `@navi-ai/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/navi-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@navi-ai/plugin/tui`.
- `@navi-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
- The value is a semver range checked against the running Navi version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.navi` is absent, no check is performed (backward compatible).
...
- `navi plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `navi plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.navi` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.navi`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.navi`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.navi` dir or `NAVI_CONFIG_DIR`, Navi installs `@navi-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@navi-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Navi's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@navi-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.navi/themes` area near the plugin config source.
...
- `--pure` / `NAVI_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.navi/plugins/tui-smoke.tsx`
- Local vim plugin: `.navi/plugins/tui-vim.tsx`
- Local smoke config: `.navi/tui.json`
- Local smoke theme: `.navi/plugins/smoke-theme.json`
