...
Delete `packages/navi/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/navi/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/navi/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/navi`
- TUI plugin loader tests in `packages/navi` if runtime plugin API wiring changed
