...
Practical reference for new and migrated Effect code in `packages/navi`.
...
export class Service extends Context.Service<Service, Interface>()("@navi/Foo") {}
...
When a service uses Effect Schema internally but needs Zod schemas for the HTTP layer, derive Zod from Schema using the `zod()` helper from `@navi-ai/core/effect-zod`:
