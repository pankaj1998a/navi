...
Practical reference for extracting a future `packages/server` from the current `packages/navi` monolith while `packages/core` is still being migrated to Effect.
...
- `packages/core` - all navi services, Effect-first source of truth
- `packages/server` - navi server, with separate contract and implementation, still producing `openapi.json`
...
- import from `server` and embed the full navi server into an existing Node app
- spawn the CLI and talk to the server through that boundary

## Current state

Everything still lives in `packages/navi`.
...
- those slices are mounted into the Hono server behind `NAVI_EXPERIMENTAL_HTTPAPI`
...
The intended direction here is the same, but the current `navi` package split is earlier in the migration.
...
- the current server host still lives in `packages/navi`
...
- if `packages/server` imports services from `packages/navi` while `packages/navi` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/navi` as the temporary Hono host
...
- `packages/server` must not import from `packages/navi`

Allowed in phase 1:

- `packages/navi` imports `packages/server`
...
- `packages/navi` shrinks or disappears as package responsibilities are fully split
...
- `packages/navi` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/navi`
...
- host mounting from `packages/navi`
...
- no imports from `packages/navi` inside `packages/server`
...
- `packages/server` must still not import from `packages/navi`
...
### PR 4. Mount `packages/server` question from `packages/navi`

Scope:

- replace local experimental question route wiring in `packages/navi`
...
- mount it in parallel from `packages/navi`
...
- move adapter selection and server startup out of `packages/navi`

Rules:

- do not start this while `packages/server` still depends on `packages/navi`
...
- host logic no longer lives in `packages/navi`
...
3. the route is mounted from the current host in `packages/navi`
...
- `packages/server` imports services or runtime from `packages/navi`
- `packages/navi` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/navi` imports
...
- [ ] mount `question` from `packages/navi`
...
2. keep `packages/navi` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/navi`, stop and restructure the boundary first.
