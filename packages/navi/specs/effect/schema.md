...
Practical reference for migrating data types in `packages/navi` from
...
schema via `@navi-ai/core/effect-zod`.
...
The walker in `@navi-ai/core/effect-zod` exposes two explicit escape hatches for
...
All of `packages/navi/src/config/` has been migrated. Files that still
...
- Use `@navi-ai/core/effect-zod` for all Schema → Zod conversion.
