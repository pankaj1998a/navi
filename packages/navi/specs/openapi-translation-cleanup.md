...
Trim `packages/navi/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/navi/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/navi`.
- `[x]` `bun typecheck` from `packages/navi`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/navi/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/navi`.
- `[x]` `bun dev generate > /tmp/navi-openapi.json` from `packages/navi`.
...
- Edit `packages/navi/src/server/routes/instance/httpapi/public.ts`.
...
- Edit route group files under `packages/navi/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/navi/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
