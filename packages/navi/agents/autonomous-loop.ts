...
import type { LocalAgentDefinition, OutputObject } from './types.js';
...
## Your Mission
Execute each story from prd.json in priority order until all pass.
    ...
    async run(executor: StoryExecutor): Promise<LoopResult> {
        ...
        if (!prd) {
            return {
                ...
                storiesCompleted: 0,
                storiesFailed: 0,
                summary: 'No prd.json found. Create a PRD first.',
            };
        }
