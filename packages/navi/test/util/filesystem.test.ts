...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "navi.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "navi.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "navi.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "navi.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["navi.json", "navi.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "navi.json"),
        path.join(tmp.path, "navi.jsonc"),
        path.join(project, "navi.json"),
        path.join(project, "navi.jsonc"),
      ])
    })
  })
