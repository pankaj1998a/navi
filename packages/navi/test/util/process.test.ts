...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.NAVI_TEST ?? "")'), {
      env: {
        NAVI_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "NAVI_TEST_SHELL"], {
      shell: true,
      env: {
        NAVI_TEST_SHELL: "ok",
      },
    ...
    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("NAVI_TEST_SHELL=ok")
  })
