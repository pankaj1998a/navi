import { $ } from "bun"
import * as Observability from "@navi-ai/core/effect/observability"
...
export async function tmpdir<T>(options?: TmpDirOptions<T>) {
  const dirpath = sanitizePath(path.join(os.tmpdir(), "navi-test-" + Math.random().toString(36).slice(2)))
    ...
    if (options?.config) {
      yield* Effect.promise(() =>
        fs.writeFile(
          ...
          JSON.stringify({ $schema: "https://navi.ai/config.json", ...options.config }),
        ),
      )
    }
