...
import { NaviClient, type GlobalEvent } from "@navi-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<NaviClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<NaviClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<NaviClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<NaviClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<NaviClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://navi.test"),
...
function sdk(
  input: {
    ...
    subscribe?: NaviClient["event"]["subscribe"]
    globalEvent?: NaviClient["global"]["event"]
    promptAsync?: NaviClient["session"]["promptAsync"]
    status?: NaviClient["session"]["status"]
    messages?: NaviClient["session"]["messages"]
    children?: NaviClient["session"]["children"]
    permissions?: NaviClient["permission"]["list"]
    questions?: NaviClient["question"]["list"]
  } = {},
) {
  const client = new NaviClient()

  const subscribe: NaviClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: NaviClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: NaviClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: NaviClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: NaviClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: NaviClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: NaviClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: NaviClient["question"]["list"] = input.questions ?? (() => ok([]))
