...
const originalNaviPort = process.env.NAVI_EDITOR_SSE_PORT

afterEach(() => {
  process.env.CLAUDE_CODE_SSE_PORT = originalClaudePort
  process.env.NAVI_EDITOR_SSE_PORT = originalNaviPort
})
...
test("useEditorContext reconnect switches editor server by session directory", async () => {
  ...
  process.env.NAVI_EDITOR_SSE_PORT = undefined
...
test("useEditorContext connects with NAVI_EDITOR_SSE_PORT", async () => {
  await using tmp = await tmpdir()
  process.env.CLAUDE_CODE_SSE_PORT = undefined
  process.env.NAVI_EDITOR_SSE_PORT = "4020"
