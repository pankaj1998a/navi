...
function request(path: string, directory: string, init: RequestInit = {}) {
  return Effect.promise(() => {
    ...
    headers.set("x-navi-directory", directory)
    ...
    Effect.gen(function* () {
      ...
      try {
        const response = yield* request(url.toString(), dir, {
          ...
          body: JSON.stringify({ $schema: "https://navi.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-navi-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-navi-workspace")
      } finally {
