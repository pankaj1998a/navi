...
const testStateLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    ...
    Flag.NAVI_EXPERIMENTAL_WORKSPACES = true
...
const directoryHeader = (dir: string) => HttpClientRequest.setHeader("x-navi-directory", dir)
...
describe("instance HttpApi", () => {
  ...
  it.live("emits a sync fence header for fixed-workspace mutations", () =>
    Effect.gen(function* () {
      const originalWorkspaceID = Flag.NAVI_WORKSPACE_ID
      Flag.NAVI_WORKSPACE_ID = WorkspaceID.ascending()
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => {
          Flag.NAVI_WORKSPACE_ID = originalWorkspaceID
        }),
      )
        ...
        Effect.promise(() =>
          ExperimentalHttpApiServer.webHandler().handler(
            new Request(`http://localhost${path}`, {
              ...init,
              headers: { "x-navi-directory": dir, "content-type": "application/json", ...init?.headers },
            }),
            handlerContext,
          ),
        )
