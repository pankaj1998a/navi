...
describe("HttpApi workspace proxy", () => {
  ...
  it.live("strips navi-internal headers and merges extra headers", () =>
    Effect.gen(function* () {
      ...
      expect(forwarded["x-navi-directory"]).toBeUndefined()
      expect(forwarded["x-navi-workspace"]).toBeUndefined()
