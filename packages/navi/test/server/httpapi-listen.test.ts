...
const auth = { username: "navi", password: "listen-secret" }
const testPty = process.platform === "win32" ? test.skip : test

afterEach(async () => {
  Flag.NAVI_SERVER_PASSWORD = original.NAVI_SERVER_PASSWORD
  Flag.NAVI_SERVER_USERNAME = original.NAVI_SERVER_USERNAME
  if (original.envPassword === undefined) delete process.env.NAVI_SERVER_PASSWORD
  else process.env.NAVI_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.NAVI_SERVER_USERNAME
  else process.env.NAVI_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.NAVI_SERVER_PASSWORD = auth.password
  Flag.NAVI_SERVER_USERNAME = auth.username
  process.env.NAVI_SERVER_PASSWORD = auth.password
  process.env.NAVI_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.NAVI_SERVER_PASSWORD = undefined
  Flag.NAVI_SERVER_USERNAME = auth.username
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-navi-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-navi-ticket": "1" }),
  ...
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-navi-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  testPty("PTY connect token requires matching directory across mint and connect", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-navi-ticket": "1" },
      })
