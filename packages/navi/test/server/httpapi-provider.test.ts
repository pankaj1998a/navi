...
function writeProviderAuthPlugin(dir: string) {
  return Effect.gen(function* () {
    ...
    yield* fs.makeDirectory(path.join(dir, ".navi", "plugin"), { recursive: true })
    yield* fs.writeFileString(
      path.join(dir, ".navi", "plugin", "provider-oauth-parity.ts"),
      [
        "export default {",
...
function writeFunctionOptionsPlugin(dir: string) {
  return Effect.gen(function* () {
    ...
    yield* fs.makeDirectory(path.join(dir, ".navi", "plugin"), { recursive: true })
    yield* fs.writeFileString(
      path.join(dir, ".navi", "plugin", "provider-function-options.ts"),
      [
        "export default {",
...
function writeProviderModelsMutationPlugin(dir: string) {
  return Effect.gen(function* () {
    ...
    yield* fs.makeDirectory(path.join(dir, ".navi", "plugin"), { recursive: true })
    yield* fs.writeFileString(
      path.join(dir, ".navi", "plugin", "provider-models-mutation.ts"),
      [
        "export default {",
...
function withProviderProject<A, E, R>(self: (dir: string) => Effect.Effect<A, E, R>) {
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem
    const path = yield* Path.Path
    const dir = yield* fs.makeTempDirectoryScoped({ prefix: "navi-test-" })

    yield* fs.writeFileString(
      path.join(dir, "navi.json"),
      JSON.stringify({ $schema: "https://navi.ai/config.json", formatter: false, lsp: false }),
    )
...
describe("provider HttpApi", () => {
  it.live(
    ...
    withProviderProject((dir) =>
      Effect.gen(function* () {
        const headers = { "x-navi-directory": dir, "content-type": "application/json" }
        const server = app()
  ...
  it.live("serves provider lists when auth loaders add runtime fetch options", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path
      const dir = yield* fs.makeTempDirectoryScoped({ prefix: "navi-test-" })
      const previous = process.env.NAVI_AUTH_CONTENT
      ...
      yield* Effect.sync(() => {
        process.env.NAVI_AUTH_CONTENT = JSON.stringify({
          google: { type: "oauth", refresh: "dummy", access: "dummy", expires: 9999999999999 },
        })
      })
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => {
          if (previous === undefined) delete process.env.NAVI_AUTH_CONTENT
          if (previous !== undefined) process.env.NAVI_AUTH_CONTENT = previous
        }),
      )
      const headers = { "x-navi-directory": dir }
