...
type Sdk = ReturnType<typeof createNaviClient>
...
function app(serverPath: ServerPath, input?: { password?: string; username?: string }) {
  Flag.NAVI_SERVER_PASSWORD = input?.password
  Flag.NAVI_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return call(() =>
    Bun.write(
      path.join(dir, ".navi", "skills", "project-rest-skill", "SKILL.md"),
      `---
      ...
      Effect.gen(function* () {
        ...
        const bad = yield* capture(() =>
          client("raw", directory, {
            password: "secret",
            headers: { authorization: authorization("navi", "wrong") },
          }).file.read({ path: "hello.txt" }),
        )
        const good = yield* capture(() =>
          client("raw", directory, {
            password: "secret",
            headers: { authorization: authorization("navi", "secret") },
          }).file.read({ path: "hello.txt" }),
        )
