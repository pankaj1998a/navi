...
const original = {
  NAVI_DISABLE_EMBEDDED_WEB_UI: Flag.NAVI_DISABLE_EMBEDDED_WEB_UI,
  NAVI_SERVER_PASSWORD: Flag.NAVI_SERVER_PASSWORD,
  NAVI_SERVER_USERNAME: Flag.NAVI_SERVER_USERNAME,
  envPassword: process.env.NAVI_SERVER_PASSWORD,
  envUsername: process.env.NAVI_SERVER_USERNAME,
}

afterEach(() => {
  Flag.NAVI_DISABLE_EMBEDDED_WEB_UI = original.NAVI_DISABLE_EMBEDDED_WEB_UI
  Flag.NAVI_SERVER_PASSWORD = original.NAVI_SERVER_PASSWORD
  Flag.NAVI_SERVER_USERNAME = original.NAVI_SERVER_USERNAME
  restoreEnv("NAVI_SERVER_PASSWORD", original.envPassword)
  restoreEnv("NAVI_SERVER_USERNAME", original.envUsername)
})
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            NAVI_SERVER_PASSWORD: input?.password,
            NAVI_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
...
describe("HttpApi UI fallback", () => {
  test("serves the web UI through the experimental backend", async () => {
    Flag.NAVI_DISABLE_EMBEDDED_WEB_UI = true
    let proxiedUrl: string | undefined

    const response = await uiApp({
      client: httpClient(
        new Response("<html>navi</html>", { headers: { "content-type": "text/html" } }),
        (request) => {
    ...
    expect(await response.text()).toBe("<html>navi</html>")
    expect(proxiedUrl).toBe("https://app.navi.ai/")
  })
  ...
  test("strips upstream transfer encoding headers from proxied assets", async () => {
    ...
    expect(response.status).toBe(200)
    expect(proxiedUrl).toBe("https://app.navi.ai/assets/app.js")
  ...
  test("requires server password for the web UI", async () => {
    ...
    const response = await uiApp({ password: "secret", username: "navi" }).request("/")
  ...
  test("accepts auth token for the web UI", async () => {
    ...
    const response = await uiApp({
      password: "secret",
      username: "navi",
      client: httpClient(new Response("<html>navi</html>", { headers: { "content-type": "text/html" } })),
    }).request(`/?auth_token=${btoa("navi:secret")}`)
  ...
  test("accepts basic auth for the web UI", async () => {
    ...
    const response = await uiApp({ password: "secret", username: "navi" }).request("/", {
      headers: { authorization: `Basic ${btoa("navi:secret")}` },
    })
  ...
  test("allows web UI preflight without auth", async () => {
    const response = await app({ password: "secret", username: "navi" }).request("/", {
      method: "OPTIONS",
      headers: {
