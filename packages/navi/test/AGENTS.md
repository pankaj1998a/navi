...
- `config?: Partial<Config.Info>` - Write an `navi.json` config file
...
- Directories are created in the system temp folder with prefix `navi-test-`
...
it.instance("uses the temp directory", () =>
  Effect.gen(function* () {
    ...
    expect(test.directory).toContain("navi-test-")
  }),
)
```
