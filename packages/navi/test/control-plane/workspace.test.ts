...
const originalWorkspacesFlag = Flag.NAVI_EXPERIMENTAL_WORKSPACES
const originalEnv = {
  NAVI_AUTH_CONTENT: process.env.NAVI_AUTH_CONTENT,
...
afterEach(async () => {
  ...
  Flag.NAVI_EXPERIMENTAL_WORKSPACES = originalWorkspacesFlag
  restoreEnv()
  await resetDatabase()
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@navi.test"`.cwd(dir).quiet()
...
describe("workspace CRUD", () => {
  ...
  test("create configures, persists, creates, starts local sync, and passes environment", async () => {
    await withInstance(async (dir) => {
      process.env.NAVI_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
      process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
      process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
      process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=navi-test"
