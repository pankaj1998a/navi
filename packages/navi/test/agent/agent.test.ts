...
async function withExperimentalScout(enabled: boolean, fn: () => Promise<void>) {
  const original = Flag.NAVI_EXPERIMENTAL_SCOUT
  Flag.NAVI_EXPERIMENTAL_SCOUT = enabled
  try {
    await fn()
  } finally {
    Flag.NAVI_EXPERIMENTAL_SCOUT = original
  }
}
...
test("plan agent denies edits except .navi/plans/*", async () => {
  await using tmp = await tmpdir()
  await WithInstance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      expect(Permission.evaluate("edit", ".navi/plans/foo.md", plan!.permission).action).toBe("allow")
    },
  })
})
...
test("skill directories are allowed for external_directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".navi", "skill", "perm-skill")
      await Bun.write(
...
`,
  ...
  const home = process.env.NAVI_TEST_HOME
  ...
  try {
    await WithInstance.provide({
      ...
      fn: async () => {
        const build = await load(tmp.path, (svc) => svc.get("build"))
        const skillDir = path.join(tmp.path, ".navi", "skill", "perm-skill")
