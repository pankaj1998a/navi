...
import { EffectFlock } from "@navi-ai/core/util/effect-flock"
...
const managedConfigDir = process.env.NAVI_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "navi.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "navi.json") {
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "navi.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  ...
  try {
    ...
    const file = path.join(tmp.path, "navi.jsonc")
...
test("ignores legacy tui keys in navi config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://navi.ai/config.json",
  ...
  try {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "navi.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["NAVI_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { navi: { options: { apiKey: "{env:NAVI_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["navi"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["NAVI_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["NAVI_CONSOLE_TOKEN"]
    }
  }
})
...
test("loads config from .navi directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const naviDir = path.join(dir, ".navi")
      await fs.mkdir(naviDir, { recursive: true })
      const agentDir = path.join(naviDir, "agent")
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".navi", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("loads agents from .navi/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const naviDir = path.join(dir, ".navi")
      await fs.mkdir(naviDir, { recursive: true })

      const agentsDir = path.join(naviDir, "agents")
...
test("loads commands from .navi/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const naviDir = path.join(dir, ".navi")
      await fs.mkdir(naviDir, { recursive: true })

      const commandDir = path.join(naviDir, "command")
...
test("loads commands from .navi/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const naviDir = path.join(dir, ".navi")
      await fs.mkdir(naviDir, { recursive: true })

      const commandsDir = path.join(naviDir, "commands")
...
test("does not try to install dependencies in read-only NAVI_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.NAVI_CONFIG_DIR
  process.env.NAVI_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.NAVI_CONFIG_DIR
    else process.env.NAVI_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable NAVI_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "navi.json"),
        JSON.stringify({ $schema: "https://navi.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .navi config
      const projectDir = path.join(dir, "project")
      const naviDir = path.join(projectDir, ".navi")
      await fs.mkdir(naviDir, { recursive: true })
      ...
      // Local .navi config with different plugins
      await Filesystem.write(
        path.join(naviDir, "navi.json"),
        JSON.stringify({
          $schema: "https://navi.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .navi config with some overlapping plugins
      await Filesystem.write(
        path.join(naviDir, "navi.json"),
        JSON.stringify({
          $schema: "https://navi.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".navi")
...
// Note: preload.ts sets NAVI_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .navi config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .navi directory config enables it
      const naviDir = path.join(dir, ".navi")
      await fs.mkdir(naviDir, { recursive: true })
      await Filesystem.write(
        path.join(naviDir, "navi.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/navi")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/navi")
    ...
    if (urlStr.includes(".well-known/navi")) {
      ...
      return Promise.resolve(
        new Response(
          JSON.stringify({
            remote_config: {
              url: "https://config.example.com/navi.json",
              headers: {
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(wellknownFetchedUrl).toBe("https://example.com/.well-known/navi")
            expect(remoteFetchedUrl).toBe("https://config.example.com/navi.json")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "navi.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-navi@2.4.3", file)).toBe("oh-my-navi@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-navi@2.4.3", "file:///project/.navi/plugin/oh-my-navi.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.navi/plugin/demo.ts", "file:///project/.navi/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.navi/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const naviDir = path.join(projectDir, ".navi")
        const pluginDir = path.join(naviDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("NAVI_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["NAVI_DISABLE_PROJECT_CONFIG"]
    process.env["NAVI_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["NAVI_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["NAVI_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .navi/ directories when flag is set", async () => {
    const originalEnv = process.env["NAVI_DISABLE_PROJECT_CONFIG"]
    process.env["NAVI_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .navi directory with a command
          const naviDir = path.join(dir, ".navi", "command")
          await fs.mkdir(naviDir, { recursive: true })
          await Filesystem.write(path.join(naviDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await WithInstance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .navi should NOT be in directories list
          const hasProjectNavi = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectNavi).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["NAVI_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["NAVI_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["NAVI_CONFIG_DIR"]
      process.env["NAVI_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["NAVI_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["NAVI_CONFIG_DIR"]
      } else {
        process.env["NAVI_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("NAVI_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["NAVI_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["NAVI_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["NAVI_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["NAVI_CONFIG_DIR"] = configDirTmp.path

      await WithInstance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from NAVI_CONFIG_DIR, not project
...
describe("NAVI_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in NAVI_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["NAVI_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["NAVI_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://navi.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["NAVI_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["NAVI_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in NAVI_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["NAVI_CONFIG_CONTENT"]
  ...
  const config = ConfigParse.effectSchema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "Navi Managed",
          PayloadIdentifier: "ai.navi.managed.test",
          PayloadType: "ai.navi.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.effectSchema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://navi.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://navi.ai/config.json")
})
