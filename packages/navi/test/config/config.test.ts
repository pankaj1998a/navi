...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "navi.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("resolves scoped npm plugins in config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "navi.json")).href
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-navi@2.4.3")).toBe("oh-my-navi")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@navi/plugin@2.0.0")).toBe("@navi/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    ...
    expect(result[0]).toBe("file:///project/.navi/plugin/oh-my-navi.js")
  })
  ...
  test("local plugin directory overrides global navi.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const naviDir = path.join(projectDir, ".navi")
