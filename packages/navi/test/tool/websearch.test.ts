...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.NAVI_WEBSEARCH_PROVIDER

    try {
      process.env.NAVI_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.NAVI_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.NAVI_WEBSEARCH_PROVIDER
      else process.env.NAVI_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is only enabled for navi or explicit websearch provider flags", () => {
    expect(webSearchEnabled(ProviderID.navi, { exa: false, parallel: false })).toBe(true)
