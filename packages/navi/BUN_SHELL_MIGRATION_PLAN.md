...
Replace runtime Bun shell template-tag usage in `packages/navi/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/navi/src` is removed except:
