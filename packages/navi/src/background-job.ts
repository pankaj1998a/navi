...
export namespace BackgroundJob {
  ...
  export class Service extends Context.Service<Service, Interface>()("@navi-ai/BackgroundJob") {}
  ...
  function pruneFinishedJobs(jobs: Map<string, Active>, maxFinished = 500): Map<string, Active> {
    const finished = Array.from(jobs.entries()).filter(([_, j]) => j.info.status !== "running")
    if (finished.length <= maxFinished) return jobs
    finished.sort((a, b) => (a[1].info.completed_at ?? 0) - (b[1].info.completed_at ?? 0))
  ...
  export const make = Effect.gen(function* () {
    ...
    const settle = Effect.fn("BackgroundJob.settle")(function* (
      ...
      const result = yield* SynchronizedRef.modify(state.jobs, (jobs): readonly [FinishResult, Map<string, Active>] => {
        ...
        const updated = pruneFinishedJobs(new Map(jobs).set(id, next))
