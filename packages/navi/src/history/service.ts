...
export class Service extends Context.Service<Service, Interface>()("@navi/History") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    const search = Effect.fn("History.search")(function* (input: Parameters<Interface["search"]>[0]) {
      ...
      if (scope === "project") {
        conditions.push("navi_history_fts.project_id = ?")
        params.push(Instance.project.id)
      }

      if (input.session_id) {
        conditions.push("navi_history_fts.session_id = ?")
        params.push(input.session_id)
      }
      if (input.kind) {
        const kinds = Array.isArray(input.kind) ? input.kind : [input.kind]
        conditions.push(`navi_history_fts.kind IN (${kinds.map(() => "?").join(",")})`)
        for (const k of kinds) params.push(k)
      }
      if (input.tool_name) {
        conditions.push("navi_history_fts.tool_name = ?")
        params.push(input.tool_name)
      }
      if (input.time_after !== undefined) {
        conditions.push("navi_history_fts.time_created >= ?")
      ...
      const sqlText = `
        SELECT navi_history_fts.part_id, navi_history_fts.session_id, navi_history_fts.message_id,
               navi_history_fts.project_id, navi_history_fts.kind, navi_history_fts.tool_name,
               navi_history_fts.time_created,
               snippet(navi_history_fts_idx, 0, '<<', '>>', '...', 32) AS snippet,
               bm25(navi_history_fts_idx) AS score
        FROM navi_history_fts_idx
        JOIN navi_history_fts ON navi_history_fts.rowid = navi_history_fts_idx.rowid
        WHERE navi_history_fts_idx MATCH ?
        ${whereClause}
        ORDER BY score
        LIMIT ?
      `
