import { tool, type Tool, jsonSchema, type JSONSchema7 } from "ai"
...
export namespace MCP {
  ...
  const state = Instance.state(
    async () => {
      ...
      if (cfg.command) {
        for (const cmd of Object.values(cfg.command)) {
          if (cmd.mcp) {
            Object.assign(config, cmd.mcp)
          }
        }
      }
      if (cfg.agent) {
        for (const agent of Object.values(cfg.agent)) {
          if (agent.mcp) {
            Object.assign(config, agent.mcp)
          }
        }
      }
