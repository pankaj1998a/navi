...
const TUI_SCHEMA_URL = "https://Navi.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from Navi.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const Navi = await NaviFiles(input)
  for (const file of Navi) {
...
async function NaviFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.NAVI_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("Navi", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "Navi")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "Navi"))
  }
  if (Flag.NAVI_CONFIG) files.push(Flag.NAVI_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "Navi"))
