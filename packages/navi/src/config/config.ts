...
import { InstallationLocal, InstallationVersion } from "@navi-ai/core/installation/version"
...
import { NonNegativeInt, PositiveInt, withStatics, type DeepMutable } from "@navi-ai/core/schema"
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for navi serve and web commands",
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://navi.ai/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://navi.ai/docs/agents" }),
...
// Uses the shared `DeepMutable` from `@navi-ai/core/schema`. See the definition
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://navi.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "navi.json")))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "NAVI_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const source = `${url}/.well-known/navi`
        ...
        if (Flag.NAVI_CONFIG) {
          yield* merge(Flag.NAVI_CONFIG, yield* loadFile(Flag.NAVI_CONFIG))
        ...
        for (const dir of directories) {
          ...
          // Auto-discovered plugins under `.navi/plugin(s)` are already local files, so ConfigPlugin.load
        ...
        if (process.env.NAVI_CONFIG_CONTENT) {
          const source = "NAVI_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.NAVI_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["NAVI_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("NAVI_CONSOLE_TOKEN", tokenOpt.value)
            }
...
export async function get() {
  ...
  return AppRuntime.runPromise(Service.use((s) => s.get()))
}
