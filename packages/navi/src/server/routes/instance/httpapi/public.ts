...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for navi serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(NaviHttpApi)
...
export const PublicApi = NaviHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "navi",
    version: "1.0.0",
