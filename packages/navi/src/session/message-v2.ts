...
export function fromError(
  ...
  switch (true) {
    ...
    case e instanceof Error && (e as { name?: string }).name === "ProviderInitError": {
      const providerID = ((e as { data?: { providerID?: string } }).data?.providerID ?? ctx.providerID) as ProviderID
      ...
      return new AuthError(
        {
          providerID,
          message: causeMsg
            ? `Failed to initialize provider "${providerID}": ${causeMsg}`
            : `Failed to initialize provider "${providerID}". Check your API key and configuration.`,
        },
        { cause: e },
      ).toObject()
    }
