...
export class Service extends Context.Service<Service, Interface>()("@navi/Memory") {}
...
export const layer: Layer.Layer<Service, never, Config.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const search = Effect.fn("Memory.search")(function* (input: {
      ...
      // See packages/navi/src/memory/fts-query.ts for the rationale.
      ...
      const params: string[] = []
      if (input.scope) {
        conditions.push("navi_memory_fts.scope = ?")
        params.push(input.scope)
      }
      if (input.scope_id) {
        conditions.push("navi_memory_fts.scope_id = ?")
        params.push(input.scope_id)
      }
      if (input.type) {
        conditions.push("navi_memory_fts.type = ?")
        params.push(input.type)
      }
      const whereClause = conditions.length > 0 ? `AND ${conditions.join(" AND ")}` : ""

      const sql = `
        SELECT navi_memory_fts.path, navi_memory_fts.scope, navi_memory_fts.scope_id, navi_memory_fts.type,
               snippet(navi_memory_fts_idx, 0, '<<', '>>', '...', 32) AS snippet,
               bm25(navi_memory_fts_idx) AS score
        FROM navi_memory_fts_idx
        JOIN navi_memory_fts ON navi_memory_fts.id = navi_memory_fts_idx.rowid
        WHERE navi_memory_fts_idx MATCH ?
        ${whereClause}
        ORDER BY score
        LIMIT ?
      `
