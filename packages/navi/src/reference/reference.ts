...
export class Service extends Context.Service<Service, Interface>()("@navi/Reference") {}
  ...
  Effect.gen(function* () {
    ...
    return Service.of({
      init: Effect.fn("Reference.init")(function* () {
        if (!Flag.NAVI_EXPERIMENTAL_SCOUT) return
      ...
      contains: Effect.fn("Reference.contains")(function* (target?: string) {
        if (!Flag.NAVI_EXPERIMENTAL_SCOUT) return false
        const full = normalizedTarget(target)
