...
import type { AssistantMessage, Event, NaviClient, SessionMessageResponse, ToolPart } from "@navi-ai/sdk/v2"
...
export function init({ sdk: _sdk }: { sdk: NaviClient }) {
  return {
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `navi auth login` in the terminal",
      name: "Login with navi",
      id: "navi-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // Navi stores files internally as { type: "file", url, filename, mime }.
