...
export namespace Skill {
  ...
  const loadSkills = Effect.fnUntraced(function* (
    ...
    for (const dir of configDirs) {
      yield* scan(state, bus, dir, NAVI_SKILL_PATTERN)
    }

    const currentDir = fileURLToPath(new URL(".", import.meta.url))
    const builtInSkillsDir = path.resolve(currentDir, "../../skills")
    if (yield* fsys.isDir(builtInSkillsDir)) {
      yield* scan(state, bus, builtInSkillsDir, "**/*/SKILL.md")
    }
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@navi/Skill") {}
