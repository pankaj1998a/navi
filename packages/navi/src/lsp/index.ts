...
export namespace LSP {
  ...
  const filterExperimentalServers = (servers: Record<string, LSPServer.Info>) => {
    if (Flag.NAVI_EXPERIMENTAL_LSP_TY) {
      // If experimental flag is enabled, disable pyright
      if (servers["pyright"]) {
        log.info("LSP server pyright is disabled because NAVI_EXPERIMENTAL_LSP_TY is enabled")
  ...
  export async function prepareRename(input: { file: string; line: number; character: number }) {
    return run(input.file, (client) =>
      client.connection
        ...
        .catch(() => null),
    ).then((result) => result.find(Boolean) || null)
  }
  ...
  export async function codeAction(input: { file: string; range: Range }) {
    return run(input.file, (client) =>
      client.connection
  ...
  export async function applyWorkspaceEdit(edit: WorkspaceEdit | null) {
    if (!edit) return { success: false, error: "No edit provided" }
    ...
    if (edit.changes) {
      for (const [uri, edits] of Object.entries(edit.changes)) {
        ...
        const res = await applyTextEditsToFile(filePath, edits)
    ...
    if (edit.documentChanges) {
      for (const change of edit.documentChanges) {
        if ("kind" in change) {
          const uri = "uri" in change ? change.uri : "newUri" in change ? change.newUri : ""
          const filePath = fileURLToPath(uri)
          try {
            if (change.kind === "create") {
              ...
              // Note: We don't delete the old file here to be safe, or we could use fs.rename
              results.push({ path: filePath, success: true })
            } else if (change.kind === "delete") {
              // Note: We don't delete files here to be safe
              results.push({ path: filePath, success: true, error: "Delete not implemented for safety" })
            }
          } catch (e) {
          ...
          const res = await applyTextEditsToFile(filePath, change.edits)
          results.push({ path: filePath, ...res })
        }
      }
    }
  ...
  async function applyTextEditsToFile(filePath: string, edits: TextEdit[]) {
    try {
      let content = await Bun.file(filePath).text()
      const lines = content.split("\n")

      // Sort edits in reverse order (bottom to top, right to left) to avoid shifting offsets
