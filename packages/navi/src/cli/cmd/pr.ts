  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
          ...
          if (prInfoText.trim()) {
            ...
            if (prInfo && prInfo.body) {
              ...
              if (sessionMatch) {
                ...
                const importResult = await Process.text(["Navi", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        const NaviProcess = Process.spawn(["Navi", ...NaviArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await NaviProcess.exited
        if (code !== 0) throw new Error(`Navi exited with code ${code}`)
      },
    })
  },
})
