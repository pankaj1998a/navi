...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run navi",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for navi session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found navi session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`navi import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting navi...")
        UI.println()

        // Launch navi TUI with session ID if available
        const { spawn } = await import("child_process")
        const naviArgs = sessionId ? ["-s", sessionId] : []
        const naviProcess = spawn("navi", naviArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          naviProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`navi exited with code ${code}`))
          })
          naviProcess.on("error", reject)
        })
      },
    })
  },
