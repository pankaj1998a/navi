...
import { createNaviClient } from "@navi-ai/sdk/v2"
...
async function runInteractiveRuntime(input: RunRuntimeInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "navi.mode": input.resolveSession ? "local" : "attach",
      "navi.initial_input": !!input.initialInput,
      "navi.demo": input.demo,
    },
    async (span) => {
      ...
      setRunSpanAttributes(span, {
        "navi.directory": ctx.directory,
        "navi.resume": ctx.resume === true,
        "navi.agent.name": state.agent,
        "navi.model.provider": state.model?.providerID,
        "navi.model.id": state.model?.modelID,
        "navi.model.variant": state.activeVariant,
      ...
      const shell = await createRuntimeLifecycle({
        ...
        onModelSelect: async (model) => {
          ...
          setRunSpanAttributes(span, {
            "navi.model.provider": model.providerID,
            "navi.model.id": model.modelID,
            "navi.model.variant": state.activeVariant,
          })
          return {
      ...
      const ensureStream = async (task: string) => {
        if (state.stream) {
          return state.stream
        }
          ...
          run: async (prompt, signal) => {
            ...
            return withRunSpan(
              "RunInteractive.turn",
              {
                ...
                "navi.model.id": state.model?.modelID,
                "navi.model.variant": state.activeVariant,
                "navi.prompt.chars": prompt.text.length,
                "navi.prompt.parts": prompt.parts.length,
                "navi.prompt.include_files": includeFiles,
                "navi.prompt.file_parts": includeFiles ? input.files.length : 0,
                "session.id": state.sessionID || undefined,
              },
              async (span) => {
                try {
                  const next = await ensureStream("prompt.turn")
                  setRunSpanAttributes(span, {
                    "navi.agent.name": state.agent,
                    "navi.model.provider": state.model?.providerID,
                    "navi.model.id": state.model?.modelID,
      ...
      try {
        ...
        if (eager) {
          await ensureStream("eager.prewarm")
        }
        ...
        if (!eager && input.resolveSession) {
          queueMicrotask(() => {
            ...
            void ensureStream("lazy.prewarm").catch(() => {})
          })
        }
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "navi.directory": input.directory,
      "navi.initial_input": !!input.initialInput,
      "navi.demo": input.demo,
    },
    async () => {
