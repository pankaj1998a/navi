...
export async function createRuntimeLifecycle(input: LifecycleInput): Promise<Lifecycle> {
  return withRunSpan(
    "RunLifecycle.boot",
    {
      "navi.agent.name": input.agent,
      "navi.directory": input.directory,
      "navi.first": input.first,
      "navi.model.provider": input.model?.providerID,
      "navi.model.id": input.model?.modelID,
      "navi.model.variant": input.variant,
    ...
    async () => {
      ...
      try {
        ...
        const { resolveRunTheme } = await import("./theme")
