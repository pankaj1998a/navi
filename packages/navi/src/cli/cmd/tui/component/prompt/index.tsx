...
import type { AssistantMessage, FilePart, UserMessage } from "@navi-ai/sdk/v2"
...
import { useBindings, useCommandShortcut, useLeaderActive, useNaviKeymap } from "../../keymap"
...
export function Prompt(props: PromptProps) {
  ...
  const promptCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        name: "workspace.set",
        category: "Session",
        enabled: Flag.NAVI_EXPERIMENTAL_WORKSPACES,
        slashName: "warp",
        run: () => {
  ...
  const spinnerDef = createMemo(() => {
    const agent = local.agent.current()
    const color = (agent ? local.agent.color(agent.name) : undefined) ?? theme.border
    return {
      frames: createFrames({
        color,
        style: "blocks",
