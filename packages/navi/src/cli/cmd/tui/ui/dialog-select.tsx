...
import { batch, createEffect, createMemo, For, Show, type JSX, on, Switch, Match } from "solid-js"
          ...
          <For each={keybinds()}>
            {(item) => (
              <box flexDirection="row" gap={1}>
                <text fg={theme.text} attributes={TextAttributes.BOLD}>{String(item.title)} </text>
                <text fg={theme.textMuted}>{String(Keybind.toString(item.keybind))}</text>
              </box>
            )}
          </For>
        </box>
    ...
    <>
      ...
      >
        ...
        <Show when={props.description}>
          <text fg={props.active ? fg : theme.textMuted}> {String(props.description)}</text>
        </Show>
      </box>
      <Show when={props.footer}>
        <box flexShrink={0}>
          <Switch>
            <Match when={typeof props.footer === 'string'}>
              <text fg={props.active ? fg : theme.textMuted}>{String(props.footer)}</text>
            </Match>
            <Match when={true}>
              {props.footer}
            </Match>
