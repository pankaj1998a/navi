...
import { BuddyHeader } from "./component/buddy-header"
...
import { DialogAgentModel } from "@tui/component/dialog-agent-model"
import { DialogBuddy } from "./component/dialog-buddy"
...
import { NaviKeymapProvider, registerNaviKeymap, useBindings, useNaviKeymap } from "./keymap"
...
function rendererConfig(_config: TuiConfig.Resolved): CliRendererConfig {
  const mouseEnabled = !Flag.NAVI_DISABLE_MOUSE && (_config.mouse ?? true)
...
function Header() {
  ...
  const species = createMemo(() => (kv.get("buddy_species", "panda") as Buddy.Species) || "panda")
  const buddy = createMemo(() => Buddy.get(species()))

  const status = createMemo(() => {
    if (route.data.type !== "session") return "idle"
    return sync.session.status(route.data.sessionID)
  })

  const buddyIcon = createMemo(() => {
    const b = buddy()
    const s = status()
    if (s === "working") return b.reactions.working
    if (s === "compacting") return b.reactions.thinking
    return b.icon
  })
...
export function tui(input: {
  ...
  return new Promise<void>(async (resolve) => {
    ...
    const offKeymap = registerNaviKeymap(keymap, renderer, input.config)
    ...
    await render(() => {
      return (
        ...
        >
          <NaviKeymapProvider keymap={keymap}>
...
function App(props: { onSnapshot?: () => Promise<string[]> }) {
  ...
  createEffect(() => {
    ...
    if (route.data.type === "plugin") {
      renderer.setTerminalTitle(`Navi | ${route.data.id}`)
    }
  })
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        slashAliases: ["ctz"],
        run: () => {
          dialog.replace(() => <DialogAgentModel />)
        },
      },
      {
        name: "buddy.list",
        title: "Switch buddy",
        desc: "Open mascot selection dialog",
        category: "Agent",
        slashName: "buddy",
        run: () => {
          dialog.replace(() => <DialogBuddy />)
        },
      },
      {
        name: "mcp.list",
  ...
  return (
    <box
      ...
      onMouseUp={Flag.NAVI_EXPERIMENTAL_DISABLE_COPY_ON_SELECT ? undefined : () => Selection.copy(renderer, toast)}
    >
      <Show when={Flag.NAVI_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
        <BuddyHeader />
