...
import { createNaviClient, type Message, type NaviClient, type ToolPart } from "@navi-ai/sdk/v2"
...
export const RunCommand = cmd({
  ...
  handler: async (args) => {
    ...
    async function session(sdk: NaviClient) {
    ...
    if (args.attach) {
      const headers = (() => {
        const password = args.password ?? process.env.NAVI_SERVER_PASSWORD
        if (!password) return undefined
        const username = process.env.NAVI_SERVER_USERNAME ?? "Navi"
        const auth = `Basic ${Buffer.from(`${username}:${password}`).toString("base64")}`
        return { Authorization: auth }
      })()
      const sdk = createNaviClient({ baseUrl: args.attach, directory, headers })
