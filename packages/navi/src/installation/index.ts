...
import { NpmConfig } from "@navi-ai/core/npm-config"
...
export const USER_AGENT = `navi/${InstallationChannel}/${InstallationVersion}/${Flag.NAVI_CLIENT}`
...
export class Service extends Context.Service<Service, Interface>()("@navi/Installation") {}
          ...
          if (detectedMethod === "npm" || detectedMethod === "bun" || detectedMethod === "pnpm") {
            const response = yield* httpOk.execute(
              HttpClientRequest.get(
                `${yield* NpmConfig.registry(process.cwd())}/navi-ai/${InstallationChannel}`,
        ...
        upgrade: Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
          ...
          switch (m) {
            case "curl":
              ...
              break
            case "npm":
              upgradeResult = yield* run(["npm", "install", "-g", `navi-ai@${target}`])
              break
            case "pnpm":
              upgradeResult = yield* run(["pnpm", "install", "-g", `navi-ai@${target}`])
              break
            case "bun":
              upgradeResult = yield* run(["bun", "install", "-g", `navi-ai@${target}`])
              break
            case "brew": {
              const formula = yield* getBrewFormula()
            ...
            case "choco":
              upgradeResult = yield* run(["choco", "upgrade", "navi", `--version=${target}`, "-y"])
              break
            case "scoop":
              upgradeResult = yield* run(["scoop", "install", `navi@${target}`])
              break
            default:
