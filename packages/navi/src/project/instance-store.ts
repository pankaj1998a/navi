...
export class Service extends Context.Service<Service, Interface>()("@navi/InstanceStore") {}
  ...
  Effect.gen(function* () {
    ...
    const provide = <A, E, R>(input: LoadInput, effect: Effect.Effect<A, E, R>): Effect.Effect<A, E, R> =>
      load(input).pipe(
        Effect.flatMap((ctx) =>
          Effect.sync(() =>
            context.provide(ctx, () => effect.pipe(Effect.provideService(InstanceRef, ctx))),
          ).pipe(Effect.flatten),
        ),
      )
