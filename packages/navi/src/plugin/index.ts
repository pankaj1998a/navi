      ...
      const cache = yield* InstanceState.make<State>(
        Effect.fn("Plugin.state")(function* (ctx) {
          ...
          const client = createNaviClient({
            ...
            headers: Flag.NAVI_SERVER_PASSWORD
              ? {
                  Authorization: `Basic ${Buffer.from(`${Flag.NAVI_SERVER_USERNAME ?? "Navi"}:${Flag.NAVI_SERVER_PASSWORD}`).toString("base64")}`,
                }
              : undefined,
          ...
          const plugins = Flag.NAVI_PURE ? [] : (cfg.plugin ?? [])
          if (Flag.NAVI_PURE && cfg.plugin?.length) {
