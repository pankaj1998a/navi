...
import { and, Database, eq, isNull, lt, or, sql } from "@navi-ai/console-core/drizzle/index.js"
import { KeyTable } from "@navi-ai/console-core/schema/key.sql.js"
import { BillingTable, SubscriptionTable, UsageTable } from "@navi-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@navi-ai/console-core/util/price.js"
import { getWeekBounds } from "@navi-ai/console-core/util/date.js"
import { Identifier } from "@navi-ai/console-core/identifier.js"
...
import { BlackData } from "@navi-ai/console-core/black.js"
...
import { ProviderTable } from "@navi-ai/console-core/schema/provider.sql.js"
...
export async function handler(
  ...
  const FREE_WORKSPACES = [
    ...
    "wrk_01K6W1A3VE0KMNVSCQT43BG2SX", // navi bench
  ]
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-navi-session") ?? ""
    const requestId = input.request.headers.get("x-navi-request") ?? ""
    const projectId = input.request.headers.get("x-navi-project") ?? ""
    const ocClient = input.request.headers.get("x-navi-client") ?? ""
    logger.metric({
      is_tream: isStream,
      session: sessionId,
      request: requestId,
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetch(reqUrl, {
        ...
        headers: (() => {
          ...
          headers.delete("x-navi-request")
          headers.delete("x-navi-session")
          headers.delete("x-navi-project")
          headers.delete("x-navi-client")
          return headers
        })(),
        body: reqBody,
      })
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo) {
    ...
    if (!billing.paymentMethodID)
      throw new CreditsError(
        `No payment method. Add a payment method here: https://navi.ai/workspace/${authInfo.workspaceID}/billing`,
      )
    if (billing.balance <= 0)
      throw new CreditsError(
        `Insufficient balance. Manage your billing here: https://navi.ai/workspace/${authInfo.workspaceID}/billing`,
      )
    ...
    if (
      ...
      throw new MonthlyLimitError(
        `Your workspace has reached its monthly spending limit of $${billing.monthlyLimit}. Manage your limits here: https://navi.ai/workspace/${authInfo.workspaceID}/billing`,
      )
    ...
    if (
      ...
      throw new UserLimitError(
        `You have reached your monthly spending limit of $${authInfo.user.monthlyLimit}. Manage your limits here: https://navi.ai/workspace/${authInfo.workspaceID}/members`,
      )
  }
