...
const model = "navi/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/navi")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with navi...`)
  ...
  try {
    await $`navi run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  navi failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with navi")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with navi...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/navi`.",
